...
PanLL (eNSAID) is a three-panel neurosymbolic AI development
environment. License: MPL-2.0. Author: Jonathan D.A. Jewell.

=== Architecture (30-second version)

* *ReScript* frontend using *TEA* (The Elm Architecture): Model -> Msg
-> Update -> View
* *Gossamer* backend (Rust + WebKitGTK) — NOT Tauri
* *106 panels* across three panes: Panel-L (Symbolic), Panel-N (Neural),
Panel-W (World)
...
Deno >= 2.0, ReScript >= 12.0, Rust/Cargo >= 1.80, Zig >= 0.13, just >=
1.25. Optional: Elixir >= 1.16 (for beam/ middleware).
...
* No npm/bun. Deno only (ReScript + Tailwind run via `+npm:+` specifiers
in `+deno.json+`).
* Panels, not panes.
...
ECHIDNA (prover), VeriSimDB (database), panic-attack (security), BoJ
server (protocol gateway), TypeLL (type verification).
