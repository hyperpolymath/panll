...
- Event-chain timeline metadata is also parsed (`timeline.duration`, `timeline.events`) and fed into `applyEventChainContents`, so Pane W can display timeline counts/duration alongside the event list, and `run_panic_attack_panll` now retains the timeline event data emitted by `panic-attack ambush` exports (with fallback conversion) so the UI sees both timeline metadata and event_chain rows even when the CLI-derived timeline is missing.
...
### P4 - Operations & Integration (User Requests)

- [ ] Configure Hypatia security gate: add `FLEET_GITHUB_TOKEN` repository secret (write access to `hyperpolymath/gitbot-fleet`), then manually `workflow_dispatch` `.github/workflows/hypatia-scan.yml` so Hypatia can submit findings to gitbot-fleet and confirm the scan metadata (`hypatia-findings.json`) is produced.
- [ ] Replace `.ctp` placeholders in `runtime/compose.toml` (`panll`, `svalinn`, `vordr`, `selur`, `rokur`) with the real Chainguard bundle references once `runtime/scripts/build-pack-verify.sh` produces `runtime/out/panll-beam.ctp` and the companion stacks are published.
- [ ] Validate the runtime stack by running `runtime/scripts/stack-up.sh` (after the bundles exist) to ensure `svalinn`, `vordr`, `selur`, `rokur`, and `panll` share the `panll` network, `PANLL_BEAM_APIS` responds over HTTP/GraphQL/gRPC, and the Chainguard `panll-beam` image proves the `BEAM -> Chainguard -> Cerro Torre` chain.
- [ ] Track `panic-attacker`'s `ambush` tool growth: verify `ambush::load_timeline_with_default` output is wired into PanLL imports, capture test coverage for the timeline execution reports, and confirm the `ambush` results are consumable by PanLL's event-chain view so the feature is no longer underdeveloped.
