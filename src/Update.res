...
// ===========================================================================
// UMS — Universal Modding Studio (unified IDApTIK content creation hub)
// ===========================================================================

/// Handles all Universal Modding Studio messages — project management,
/// ABI validation, template instantiation, asset pipeline, and mod distribution.
let updateUms = (model: model, msg: umsMsg): (model, Tea_Cmd.t<msg>) => {
  ...
  | ProjectsLoaded(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        arr->Array.filterMap(item => {
          ...
          let version = obj->Dict.get("version")->Option.flatMap(JSON.Decode.string)->Option.getOr("0.1.0")
          let createdAt = obj->Dict.get("createdAt")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let lastModified = obj->Dict.get("lastModified")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let levelCount = obj->Dict.get("levelCount")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)->Float.toInt
          let puzzleCount = obj->Dict.get("puzzleCount")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)->Float.toInt
          let assetCount = obj->Dict.get("assetCount")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)->Float.toInt
          let validated = obj->Dict.get("validated")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
          let projectPath = obj->Dict.get("projectPath")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          if id !== "" {
            Some({UmsModel.id, name, description, author, version, createdAt, lastModified, levelCount, puzzleCount, assetCount, validated, projectPath})
          } else {
            None
          }
        })
  ...
  | ProjectOpened(Error(err)) => ({...model, ums: {...u, loading: false, error: Some(err)}}, Tea_Cmd.none)
  | DeleteProject(id) => (
      {...model, ums: {...u, loading: true}},
  ...
  | ProjectDeleted(Error(err)) => ({...model, ums: {...u, loading: false, error: Some(err)}}, Tea_Cmd.none)
  | ValidateLevel(levelId) => {
      ...
      let validateCmd = if u.bojRouting {
        BojCmd.invokeCartridgeWithLatency("database-mcp", "ums_validate_level", levelId, result => Ums(ValidationResult(result)), (c, t, e) => RecordBojLatency(c, t, e))
      } else {
      ...
      (
        {...model, ums: {...u, loading: true}},
        Tea_Cmd.batch(list{validateCmd, typellCmd}),
      )
    }
  | ValidationResult(Ok(jsonStr)) => {
      let parsed = switch Decoders.decodeOption(Tea_Json.value, jsonStr) {
      | Some(json) =>
        ...
        let defenceTargetsValid = obj->Dict.get("defenceTargetsValid")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
        let zonesOrdered = obj->Dict.get("zonesOrdered")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
        let pbxConsistent = obj->Dict.get("pbxConsistent")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
        let devicesExist = obj->Dict.get("devicesExist")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
        let allPassed = guardsInZones && defenceTargetsValid && zonesOrdered && pbxConsistent && devicesExist
        let validatedAt = obj->Dict.get("validatedAt")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
        let errors = obj->Dict.get("errors")->Option.flatMap(JSON.Decode.array)->Option.getOr([])->Array.filterMap(e =>
      ...
      ({...model, ums: {...u, validationResults: results, loading: false, error: None}}, Tea_Cmd.none)
    }
  | ValidationResult(Error(err)) => ({...model, ums: {...u, loading: false, error: Some(err)}}, Tea_Cmd.none)
  | LoadTemplates => (
      {...model, ums: {...u, loading: true}},
  ...
  | ApiReferenceLoaded(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        arr->Array.filterMap(item => {
          ...
          let name = obj->Dict.get("name")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let category = obj->Dict.get("category")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let signature = obj->Dict.get("signature")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let description = obj->Dict.get("description")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let example = obj->Dict.get("example")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
  ...
  | ApiReferenceLoaded(Error(err)) => ({...model, ums: {...u, loading: false, error: Some(err)}}, Tea_Cmd.none)
