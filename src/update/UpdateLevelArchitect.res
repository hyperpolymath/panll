...
let updateLevelArchitect = (model: model, msg: levelArchitectMsg): (model, Tea_Cmd.t<msg>) => {
  ...
  switch msg {
  | SetArchitectCategory(cat) => (
      {...model, levelArchitect: {...la, activeCategory: cat}},
      Tea_Cmd.none,
    )
  | ClickGrid(x, y) => // Behaviour depends on selected tool
    ...
    | ToolPatrol => {
        ...
        | Some(guardId) => {
            ...
            | None =>
              Array.concat(
                la.patrols,
                [{guardId, waypoints: [waypoint], looping: true, speed: 1.0}],
              )
            }
          }
        | None => la.patrols
    ...
    | ToolDefenceFlag => // Place a defence flag entity at the clicked grid position.
      if !LevelArchitectEngine.isOccupied(la.entities, x, y) {
  ...
  | SelectEntity(id) => (
      {...model, levelArchitect: {...la, selectedEntityId: Some(id)}},
      Tea_Cmd.none,
    )
  ...
  | AssetsLoaded(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        let items = arr->Array.filterMap(item => {
          ...
          let thumbnail =
            obj->Dict.get("thumbnail")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let kindStr =
            obj->Dict.get("entityKind")->Option.flatMap(JSON.Decode.string)->Option.getOr("device")
      ...
      switch parsed {
      | Some(assets) => (
          {...model, levelArchitect: {...la, assets, loading: false, error: None}},
          Tea_Cmd.none,
        )
  ...
  | ValidateLevel => (
      ...
      Tea_Cmd.batch(list{
        ...
        TypeLLService.checkGameDataTypes("level-data", "level-architect", result => LevelArchitect(
  ...
  | ValidationResult(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        let issues = arr->Array.filterMap(item => {
          ...
          let gridX =
            obj->Dict.get("gridX")->Option.flatMap(JSON.Decode.float)->Option.map(Float.toInt)
          let gridY =
            obj->Dict.get("gridY")->Option.flatMap(JSON.Decode.float)->Option.map(Float.toInt)
          Some({
            LevelArchitectModel.severity,
            message,
            entityId,
            gridX,
            gridY,
      ...
      | Some(issues) => (
          {
            ...model,
            levelArchitect: {...la, validationIssues: issues, loading: false, error: None},
          },
          Tea_Cmd.none,
        )
      | None => ({...model, levelArchitect: {...la, loading: false, error: None}}, Tea_Cmd.none)
  ...
  | LevelLoaded(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        let levelName =
          obj->Dict.get("levelName")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
        let gridWidth =
          obj->Dict.get("gridWidth")->Option.flatMap(JSON.Decode.float)->Option.getOr(32.0)
        let gridHeight =
          obj->Dict.get("gridHeight")->Option.flatMap(JSON.Decode.float)->Option.getOr(32.0)
        ...
        let entities = entArr->Array.filterMap(e => {
          ...
          let kindStr =
            eObj->Dict.get("kind")->Option.flatMap(JSON.Decode.string)->Option.getOr("device")
          ...
          let rotation =
            eObj->Dict.get("rotation")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let propsArr =
            eObj->Dict.get("properties")->Option.flatMap(JSON.Decode.array)->Option.getOr([])
        ...
        let patrols = patArr->Array.filterMap(p => {
          ...
          let guardId =
            pObj->Dict.get("guardId")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let looping =
            pObj->Dict.get("looping")->Option.flatMap(JSON.Decode.bool)->Option.getOr(true)
          let speed = pObj->Dict.get("speed")->Option.flatMap(JSON.Decode.float)->Option.getOr(1.0)
          let wpArr =
            pObj->Dict.get("waypoints")->Option.flatMap(JSON.Decode.array)->Option.getOr([])
          let waypoints = wpArr->Array.filterMap(w => {
            ...
            let pauseDuration =
              wObj->Dict.get("pauseDuration")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
            Some({LevelArchitectModel.x: Float.toInt(x), y: Float.toInt(y), pauseDuration})
          })
          Some({
            LevelArchitectModel.guardId,
            waypoints,
            looping,
            speed,
          })
  ...
  | LevelSaved(Error(err)) => ({...model, levelArchitect: {...la, error: Some(err)}}, Tea_Cmd.none)
  | ExportLevelConfig => (
      model,
      LevelArchitectCmd.exportLevelConfig("", result => LevelArchitect(
        LevelConfigExported(result),
      )),
    )
  ...
  | UndoAction => if la.historyIndex > 0 {
  ...
  | RedoAction => if la.historyIndex < Array.length(la.history) - 1 {
      let newIdx = la.historyIndex + 1
      let entry = la.history->Array.get(newIdx)
      switch entry {
      | Some(e) => (
