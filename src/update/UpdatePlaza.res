...
let updatePlaza = (model: model, msg: plazaMsg): (model, Tea_Cmd.t<msg>) => {
  ...
  | RepoScanned(result) =>
    ...
    | Ok(jsonStr) => {
                ...
                checks: [
                  {
                    ...
                    passed: hl,
                    severity: "critical",
                    detail: hl ? "Found" : "Missing",
                  },
                  {
                    id: "spdx",
                    name: "SPDX",
                    description: "SPDX headers",
                    passed: sc === tf,
                    severity: "warning",
                    detail: `${Int.toString(sc)}/${Int.toString(tf)}`,
                  },
                  {
                    id: "exhibit-a",
                    name: "Exhibit A",
                    description: "Ethical Use",
                    passed: ea,
                    severity: "info",
                    detail: ea ? "Present" : "Missing",
                  },
                  {
                    id: "exhibit-b",
                    name: "Exhibit B",
                    description: "QS Provenance",
                    passed: eb,
                    severity: "info",
                    detail: eb ? "Present" : "Missing",
                  },
                  {
                    id: "sig",
                    name: "Provenance",
        ...
        switch audit {
        | Some(a) => (
            {...model, plaza: {...plaza, loading: false, audits: Array.concat(plaza.audits, [a])}},
            Tea_Cmd.none,
          )
  ...
  | SetPlazaCategory(cat) => ({...model, plaza: {...plaza, activeCategory: cat}}, Tea_Cmd.none)
  | SetPlazaFilter(text) => ({...model, plaza: {...plaza, filterText: text}}, Tea_Cmd.none)
