...
let updateMigration = (model: model, msg: migrationMsg): (model, Tea_Cmd.t<msg>) => {
  ...
  switch msg {
  | LoadMigrationData => (
      {...model, migration: {...mig, loading: true, error: None}},
      TypeLLService.checkMetadataTypes("migration-data", "migration", result => Migration(
        TypeCheckResult(result),
      )),
    )
  | MigrationDataLoaded(Ok(_data)) => // Data parsing would happen here in a real implementation.
  ...
  | MigrationDataLoaded(Error(err)) => (
      {...model, migration: {...mig, loading: false, error: Some(err)}},
      Tea_Cmd.none,
    )
  | SetMigrationCategory(cat) => (
      {...model, migration: {...mig, activeCategory: cat}},
      Tea_Cmd.none,
    )
  | SetMigrationReportType(rt) => (
      {...model, migration: {...mig, activeReportType: rt}},
      Tea_Cmd.none,
    )
  | SetMigrationFilter(text) => ({...model, migration: {...mig, filterText: text}}, Tea_Cmd.none)
  | BeginObservation(_repo, _label) => (
      {...model, migration: {...mig, loading: true}},
      Tea_Cmd.none,
    )
  | ObservationStarted(Ok(_data)) => ({...model, migration: {...mig, loading: false}}, Tea_Cmd.none)
  | ObservationStarted(Error(err)) => (
      {...model, migration: {...mig, loading: false, error: Some(err)}},
      Tea_Cmd.none,
    )
  | EndObservation(_sessionId) => ({...model, migration: {...mig, loading: true}}, Tea_Cmd.none)
  | ObservationEnded(Ok(_data)) => ({...model, migration: {...mig, loading: false}}, Tea_Cmd.none)
  ...
  | SubmitApproved => ({...model, migration: {...mig, loading: true}}, Tea_Cmd.none)
  ...
  | MergeResolutionStarted(Ok(_data)) => (
      {...model, migration: {...mig, loading: false}},
      Tea_Cmd.none,
    )
  ...
  | RollbackMerge(_sessionId) => (model, Tea_Cmd.none)
  | AcceptMerge(_sessionId) => (model, Tea_Cmd.none)
  | RefreshMigrationHealth => ({...model, migration: {...mig, loading: true}}, Tea_Cmd.none)
