...
let updateMultiplayerMonitor = (model: model, msg: multiplayerMonitorMsg): (
  model,
  Tea_Cmd.t<msg>,
) => {
  ...
  | ConnectServer => (
      ...
      Tea_Cmd.batch(list{
        MultiplayerMonitorCmd.connectToServer(mp.serverUrl, result => MultiplayerMonitor(
          ConnectionResult(result),
        )),
        TypeLLService.checkGameDataTypes(mp.serverUrl, "multiplayer", result => MultiplayerMonitor(
  ...
  | DisconnectServer => (
      ...
      MultiplayerMonitorCmd.disconnectFromServer(result => MultiplayerMonitor(
  ...
  | ConnectionResult(Ok(_)) => (
      {
        ...model,
        multiplayerMonitor: {...mp, wsConnection: WsConnected, loading: false, error: None},
      },
      MultiplayerMonitorCmd.readMultiplayerState(result => MultiplayerMonitor(
  ...
  | ConnectionResult(Error(err)) => (
      {
        ...model,
        multiplayerMonitor: {...mp, wsConnection: WsError(err), loading: false, error: Some(err)},
      },
      Tea_Cmd.none,
    )
  ...
  | StateReceived(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        let playersArr =
          obj->Dict.get("players")->Option.flatMap(JSON.Decode.array)->Option.getOr([])
        let players = playersArr->Array.filterMap(item => {
          let p = item->JSON.Decode.object->Option.getOr(Dict.make())
          let playerId =
            p->Dict.get("playerId")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let displayName =
            p->Dict.get("displayName")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let deviceId =
            p->Dict.get("deviceId")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let latencyMs =
            p->Dict.get("latencyMs")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let lamportClock =
            p->Dict.get("lamportClock")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let lastHeartbeat =
            p->Dict.get("lastHeartbeat")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let isHost = p->Dict.get("isHost")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
          let isSpectator =
            p->Dict.get("isSpectator")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
          Some({
        ...
        let channelsArr =
          obj->Dict.get("channels")->Option.flatMap(JSON.Decode.array)->Option.getOr([])
        let channels = channelsArr->Array.filterMap(item => {
          ...
          let joinedAt =
            c->Dict.get("joinedAt")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let messageCount =
            c->Dict.get("messageCount")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let lastMessageAt =
            c->Dict.get("lastMessageAt")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          Some({
        ...
        let locksArr =
          obj->Dict.get("deviceLocks")->Option.flatMap(JSON.Decode.array)->Option.getOr([])
        let deviceLocks = locksArr->Array.filterMap(item => {
          let l = item->JSON.Decode.object->Option.getOr(Dict.make())
          let deviceId =
            l->Dict.get("deviceId")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let lockedBy = l->Dict.get("lockedBy")->Option.flatMap(JSON.Decode.string)
          let lockedAt =
            l->Dict.get("lockedAt")->Option.flatMap(JSON.Decode.float)->Option.getOr(0.0)
          let contestedArr =
            l->Dict.get("contestedBy")->Option.flatMap(JSON.Decode.array)->Option.getOr([])
  ...
  | DiffsReceived(Ok(jsonStr)) => {
      ...
      | Some(json) =>
        ...
        let diffs = arr->Array.filterMap(item => {
          ...
          let playerId =
            obj->Dict.get("playerId")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let field = obj->Dict.get("field")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let localValue =
            obj->Dict.get("localValue")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let remoteValue =
            obj->Dict.get("remoteValue")->Option.flatMap(JSON.Decode.string)->Option.getOr("")
          let resolved =
            obj->Dict.get("resolved")->Option.flatMap(JSON.Decode.bool)->Option.getOr(false)
          Some({
      ...
      switch parsed {
      | Some(stateDiffs) => (
          {...model, multiplayerMonitor: {...mp, stateDiffs, loading: false, error: None}},
          Tea_Cmd.none,
        )
      | None => ({...model, multiplayerMonitor: {...mp, loading: false, error: None}}, Tea_Cmd.none)
      }
  ...
  | ReconnectionTest => (
      {...model, multiplayerMonitor: {...mp, loading: true}},
      MultiplayerMonitorCmd.reconnectionTest(result => MultiplayerMonitor(
        ReconnectionTestResult(result),
      )),
    )
  ...
  | DismissMultiplayerError => ({...model, multiplayerMonitor: {...mp, error: None}}, Tea_Cmd.none)
