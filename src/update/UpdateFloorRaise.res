...
let updateOo7Toolchain = (model: model, msg: oo7Msg): (model, Tea_Cmd.t<msg>) => {
  ...
  | ConnectDaemon => ({...model, oo7toolchain: {...state, loading: true, error: None}}, Tea_Cmd.none)
  ...
  | StageResult(stage, result) =>
    ...
    | Error(err) => ({...model, oo7toolchain: {...state, loading: false, error: Some(err)}}, Tea_Cmd.none)
    }
