          ...
          div(
            ...
            list{
              button(
                list{
                  Attrs.class_(
                    "px-3 py-1 text-xs rounded " ++ if state.activeTab == EchidnaProofTab {
              ...
              button(
                list{
                  Attrs.class_(
                    "px-3 py-1 text-xs rounded " ++ if state.activeTab == EchidnaEnterpriseTab {
                      "bg-purple-700 text-white"
                    } else {
                      "bg-gray-800 text-gray-400"
                    },
        ...
        list{
            ...
            div(
              ...
              list{
                ...
                div(
                  ...
                  list{
                    label(
                      list{Attrs.class_("text-xs text-gray-400 block mb-1")},
                      list{text("Proof Obligation")},
                    ),
                    textarea(
                      list{
                        Attrs.class_(
                          "w-full h-40 bg-gray-900 border border-gray-700 rounded p-2 text-sm text-gray-200 font-mono",
                        ),
                      ...
                      div(
                        ...
                        ->Array.map(s =>
                          span(
                            list{
                              Attrs.class_(
                                "px-2 py-1 text-xs bg-purple-900/50 border border-purple-700 rounded cursor-pointer hover:bg-purple-800",
                              ),
                            },
                            list{
                              text(
                                s.tactic ++
                                " (" ++
                                Float.toFixed(s.confidence *. 100.0, ~digits=0) ++ "%)",
                              ),
                            },
                          )
                        )
          ...
          | EchidnaEnterpriseTab =>
            div(
              ...
              list{
                text(
                  "MOF/OCL enterprise model checking — load XMI models and verify OCL constraints.",
                ),
              },
            )
          },
