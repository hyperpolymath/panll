    ...
    list{
      div(list{Attrs.class_("flex items-center justify-between")}, list{
        ...
        div(list{Attrs.class_("flex items-center gap-1")}, list{
          span(list{Attrs.class_("text-xs font-mono " ++ scoreColor)}, list{
            text(Float.toFixed(tool.overallScore, ~digits=1)),
          }),
...
let dimensionBar = (dim: isaDimension): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      ...
      div(list{Attrs.class_("flex-1 bg-gray-800 rounded-full h-2")}, list{
        div(
          list{
      ...
      span(list{Attrs.class_("text-xs text-gray-500 w-8 text-right")}, list{
        text(Float.toFixed(dim.score, ~digits=1)),
      }),
      span(list{Attrs.class_("text-xs text-gray-600 w-10 text-right")}, list{
        text(`(${Int.toString(dim.sampleCount)})`),
      }),
    },
  )
}
...
/// Render a tab button.
let tabBtn = (current: vexometerFrictionTab, target: vexometerFrictionTab, label: string): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      ...
      div(
        ...
        list{
          div(list{Attrs.class_("flex items-center gap-3")}, list{
            h2(list{Attrs.class_("text-lg font-bold text-orange-300")}, list{text("Vexometer Friction")}),
            span(list{Attrs.class_("text-xs text-gray-500")}, list{
              text(`Avg: ${Float.toFixed(avgFriction, ~digits=1)}/10.0 | ${Int.toString(total)} tools`),
            }),
          }),
          button(
            list{
            ...
            list{text(if state.measuring {"Measuring..."} else {"Measure All"})},
          ),
        },
      ),
      // Tabs
      div(
        ...
        VexometerFrictionEngine.allTabs->Array.map(t =>
          tabBtn(state.activeTab, t, VexometerFrictionEngine.tabLabel(t))
        )->List.fromArray,
      ),
      // Error banner
      switch state.error {
      ...
      div(
        ...
        list{
          ...
          div(
            ...
            VexometerFrictionEngine.sortByFriction(state.tools)->Array.map(t =>
              toolRow(t, state.selectedTool == Some(t.toolName))
            )->List.fromArray,
          ),
          ...
          div(
            ...
            list{
              ...
              | Some(name) =>
                ...
                | Some(tool) =>
                  div(list{}, list{
                    div(list{Attrs.class_("flex items-center justify-between mb-3")}, list{
                      h3(list{Attrs.class_("text-md font-semibold text-gray-200")}, list{text(tool.toolName)}),
                      div(list{Attrs.class_("flex items-center gap-2")}, list{
                        span(list{Attrs.class_("text-sm font-mono text-gray-300")}, list{
                          text(`${Float.toFixed(tool.overallScore, ~digits=1)}/10.0`),
                        }),
                        trendIndicator(tool.trend),
                        span(list{Attrs.class_("text-xs text-gray-500")}, list{
                          text(VexometerFrictionEngine.trendLabel(tool.trend)),
                        }),
                      }),
                    }),
                    ...
                    div(list{Attrs.class_("mt-3 text-xs text-gray-600")}, list{
                      text(`Last measured: ${tool.lastMeasured}`),
                    }),
                  })
                }
              },
      ...
      div(
        ...
        list{
          text(
            `${Int.toString(state.tools->Array.filter(t => t.trend == Worsening)->Array.length)} tools worsening`,
          ),
        },
      ),
    },
