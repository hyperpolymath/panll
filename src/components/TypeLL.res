...
let renderViewLayerSelector = (active: viewLayer): Tea_Vdom.t<msg> => {
  div(
    ...
    TypeLLEngine.allViewLayers->Array.map(vl => {
...
let renderFeatureBadge = (f: typeFeature, viewLayer: viewLayer): Tea_Vdom.t<msg> => {
  ...
  | Wysiwyg =>
    div(
      ...
      list{
        div(list{Attrs.class_("font-medium")}, list{text(`${glyph.symbol} ${TypeLLEngine.featureLabel(f)}`)}),
    ...
    list{
      if narrative.celebrate !== "" {
        div(
          ...
          list{
            span(list{Attrs.class_("text-emerald-400 text-sm font-medium shrink-0")}, list{text("Celebrate")}),
      ...
      if narrative.minimize !== "" {
        div(
          list{Attrs.class_("flex items-start gap-2")},
          list{
            span(list{Attrs.class_("text-amber-400 text-sm font-medium shrink-0")}, list{text("Note")}),
      ...
      if narrative.showBetter !== "" {
        div(
          list{Attrs.class_("flex items-start gap-2")},
          list{
            span(list{Attrs.class_("text-cyan-400 text-sm font-medium shrink-0")}, list{text("Better")}),
      ...
      if narrative.safety !== "" {
        div(
          list{Attrs.class_("flex items-start gap-2")},
          list{
            span(list{Attrs.class_("text-violet-400 text-sm font-medium shrink-0")}, list{text("Safety")}),
...
let renderCheckResult = (result: typeCheckResult, viewLayer: viewLayer, narrative: option<typeNarrative>): Tea_Vdom.t<msg> => {
  div(
    ...
    list{
      ...
      div(
        ...
        list{
          div(
            ...
            list{
              span(
                list{Attrs.class_(if result.valid { "text-emerald-400 text-sm font-medium" } else { "text-red-400 text-sm font-medium" })},
                list{text(if result.valid { "Well-typed" } else { "Type error" })},
              ),
              span(
                list{Attrs.class_(`px-2 py-0.5 text-xs rounded ${TypeLLEngine.tierColour(result.maxTier)}`)},
          ...
          pre(
            list{Attrs.class_("font-mono text-sm text-gray-200 bg-gray-950 rounded p-3 whitespace-pre-wrap")},
            list{text(TypeLLEngine.formatSignature(result.typeSignature, viewLayer, result.activeFeatures))},
          ),
      ...
      if result.proofObligations->Array.length > 0 {
        div(
          ...
          list{
            div(list{Attrs.class_("text-xs text-violet-400 mb-2 font-medium")}, list{text("Proof Obligations")}),
            div(
      ...
      if result.effects->Array.length > 0 {
        div(
          ...
          list{
            div(list{Attrs.class_("text-xs text-amber-400 mb-2 font-medium")}, list{text("Effects")}),
            div(
              list{Attrs.class_("flex flex-wrap gap-2")},
              result.effects->Array.map(e =>
                span(list{Attrs.class_("px-2 py-0.5 text-xs rounded bg-amber-900/30 text-amber-300")}, list{text(e)})
              )->List.fromArray,
            ),
          },
        )
      ...
      if result.linearityIssues->Array.length > 0 {
        div(
          ...
          list{
            div(list{Attrs.class_("text-xs text-red-400 mb-2 font-medium")}, list{text("Linearity Issues")}),
            div(
              list{Attrs.class_("space-y-1")},
              result.linearityIssues->Array.map(li =>
                div(list{Attrs.class_("text-sm text-red-300")}, list{text(li)})
              )->List.fromArray,
            ),
          },
        )
...
let renderTabs = (active: typellCategory): Tea_Vdom.t<msg> => {
  div(
    ...
    TypeLLEngine.allCategories->Array.map(tab => {
    ...
    list{
      ...
      div(
        ...
        list{
            ...
            div(
              ...
              list{
                div(
                  ...
                  list{text("Enter an expression to type-check. TypeLL detects dependent, linear, affine, session, and refinement types automatically.")},
                ),
                textarea(
                  list{
                    Attrs.class_("w-full h-40 bg-gray-900 border border-gray-700 rounded-lg p-4 font-mono text-sm text-gray-200 resize-none focus:border-cyan-500 focus:outline-none"),
                ...
                div(
                  ...
                  list{
                    button(
                      list{
                        Attrs.class_("px-4 py-2 text-sm bg-cyan-600 text-white rounded hover:bg-cyan-500"),
                    ...
                    button(
                      list{
                        Attrs.class_("px-4 py-2 text-sm bg-gray-800 text-gray-300 rounded hover:bg-gray-700"),
              ...
              list{
                div(
                  ...
                  list{
                    input(
                      list{
                        Attrs.class_("flex-1 bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 placeholder-gray-600 font-mono"),
                    ...
                    button(
                      list{
                        Attrs.class_(
                          `px-3 py-1.5 text-xs rounded ${tl.tierFilter === None ? "bg-gray-700 text-gray-200" : "text-gray-500 hover:text-gray-300"}`,
                        ),
                      ...
                      list{
                        Attrs.class_(
                          `px-3 py-1.5 text-xs rounded ${tl.tierFilter === Some(TierCore)
                              ? TypeLLEngine.tierColour(TierCore) : "text-gray-500 hover:text-gray-300"}`,
                        ),
                      ...
                      list{
                        Attrs.class_(
                          `px-3 py-1.5 text-xs rounded ${tl.tierFilter === Some(TierAdvanced)
                              ? TypeLLEngine.tierColour(TierAdvanced) : "text-gray-500 hover:text-gray-300"}`,
                        ),
                      ...
                      list{
                        Attrs.class_(
                          `px-3 py-1.5 text-xs rounded ${tl.tierFilter === Some(TierResearch)
                              ? TypeLLEngine.tierColour(TierResearch) : "text-gray-500 hover:text-gray-300"}`,
                        ),
                    ...
                    button(
                      list{
                        Attrs.class_("px-3 py-1.5 text-xs bg-cyan-600 text-white rounded hover:bg-cyan-500"),
                  ...
                  div(
                    ...
                    list{
                      div(
                        ...
                        filtered->Array.map(sig =>
                          div(
                            list{Attrs.class_("flex items-center gap-3 p-3 border-b border-gray-800 hover:bg-gray-900/50")},
                            list{
                              span(
                                list{Attrs.class_(`px-1.5 py-0.5 text-xs rounded ${TypeLLEngine.tierColour(sig.tier)}`)},
                                list{text(TypeLLEngine.tierLabel(sig.tier))},
                              ),
                              span(list{Attrs.class_("text-sm font-mono text-cyan-400")}, list{text(sig.name)}),
                              span(list{Attrs.class_("text-sm font-mono text-gray-400 flex-1 truncate")}, list{text(sig.signature)}),
                              span(list{Attrs.class_("text-xs text-gray-600")}, list{text(sig.module_)}),
                            },
                          )
                        )->List.fromArray,
                      ),
                ...
                if tl.universes->Array.length > 0 {
                  div(
                    ...
                    list{
                      h3(list{Attrs.class_("text-sm font-medium text-gray-300 mb-3")}, list{text("Type Universes")}),
                      div(
                        list{Attrs.class_("space-y-2")},
                        tl.universes->Array.map(u =>
                          div(
                            list{Attrs.class_("flex items-center gap-3 text-sm")},
                            list{
                              span(list{Attrs.class_("font-mono text-violet-400 w-8 text-right")}, list{text(Int.toString(u.level))}),
                              span(list{Attrs.class_("font-mono text-gray-200")}, list{text(u.name)}),
          ...
          | TlRefinement =>
            div(
              ...
              list{
                div(
                  list{Attrs.class_("text-sm text-gray-400 mb-2")},
                  list{text("Narrow a type with refinement constraints. TypeLL checks satisfiability and consistency.")},
                ),
                div(
                  ...
                  list{
                    div(
                      ...
                      list{
                        ...
                        input(
                          list{
                            Attrs.class_("w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 font-mono placeholder-gray-600"),
                    ...
                    div(
                      list{Attrs.class_("space-y-2")},
                      list{
                        label(list{Attrs.class_("text-xs text-gray-500")}, list{text("Constraints (one per line)")}),
                        textarea(
                          list{
                            Attrs.class_("w-full h-20 bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 font-mono resize-none placeholder-gray-600"),
                ...
                button(
                  list{
                    Attrs.class_("px-4 py-2 text-sm bg-violet-600 text-white rounded hover:bg-violet-500"),
                  ...
                  div(
                    ...
                    list{
                      div(
                        ...
                        list{
                          span(
                            list{Attrs.class_(if ref.consistent { "text-emerald-400 text-sm" } else { "text-red-400 text-sm" })},
                            list{text(if ref.consistent { "Consistent" } else { "Inconsistent" })},
                          ),
                        },
                      ),
                      div(
                        ...
                        list{
                          div(list{}, list{
                            div(list{Attrs.class_("text-xs text-gray-500 mb-1")}, list{text("Base Type")}),
                            pre(list{Attrs.class_("font-mono text-sm text-gray-300 bg-gray-950 rounded p-2")}, list{text(ref.baseType)}),
                          }),
                          div(list{}, list{
                            div(list{Attrs.class_("text-xs text-gray-500 mb-1")}, list{text("Refined Type")}),
                            pre(list{Attrs.class_("font-mono text-sm text-cyan-300 bg-gray-950 rounded p-2")}, list{text(ref.refinedType)}),
                          }),
                        },
                      ),
                    },
            ...
            div(
              ...
              list{
                ...
                div(
                  ...
                  list{
                    div(
                      ...
                      list{
                        span(list{Attrs.class_(`px-2 py-0.5 text-xs font-medium rounded ${TypeLLEngine.tierColour(TierCore)}`)}, list{text("Tier 1: Core")}),
                        span(list{Attrs.class_("text-sm text-gray-400")}, list{text("Essential type safety")}),
                      },
                    ),
                    div(
                      list{Attrs.class_("grid grid-cols-2 gap-3")},
                      TypeLLEngine.coreFeatures->Array.map(f => {
                        let glyph = TypeLLEngine.featureGlyph(f)
                        div(
                          ...
                          list{
                            div(list{Attrs.class_("flex items-center gap-2 mb-1")}, list{
                              ...
                              span(list{Attrs.class_("text-sm font-medium text-gray-200")}, list{text(TypeLLEngine.featureLabel(f))}),
                            }),
                            div(list{Attrs.class_("text-xs text-gray-500")}, list{text(glyph.meaning)}),
                          },
                        )
                      })->List.fromArray,
                    ),
                ...
                div(
                  ...
                  list{
                    div(
                      list{Attrs.class_("flex items-center gap-2")},
                      list{
                        span(list{Attrs.class_(`px-2 py-0.5 text-xs font-medium rounded ${TypeLLEngine.tierColour(TierAdvanced)}`)}, list{text("Tier 2: Advanced")}),
                        span(list{Attrs.class_("text-sm text-gray-400")}, list{text("Precision resource management")}),
                      },
                    ),
                    div(
                      list{Attrs.class_("grid grid-cols-2 gap-3")},
                      TypeLLEngine.advancedFeatures->Array.map(f => {
                        let glyph = TypeLLEngine.featureGlyph(f)
                        div(
                          list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3")},
                          list{
                ...
                div(
                  ...
                  list{
                    div(
                      list{Attrs.class_("flex items-center gap-2")},
                      list{
                        span(list{Attrs.class_(`px-2 py-0.5 text-xs font-medium rounded ${TypeLLEngine.tierColour(TierResearch)}`)}, list{text("Tier 3: Research")}),
                        span(list{Attrs.class_("text-sm text-gray-400")}, list{text("Frontier type theory")}),
                      },
                    ),
                    div(
                      list{Attrs.class_("grid grid-cols-2 gap-3")},
                      TypeLLEngine.researchFeatures->Array.map(f => {
                        let glyph = TypeLLEngine.featureGlyph(f)
                        div(
                          list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3")},
                          list{
                ...
                div(
                  list{Attrs.class_("space-y-3")},
                  list{
                    h3(list{Attrs.class_("text-sm font-medium text-gray-200")}, list{text("Cross-Panel Integration")}),
                    div(list{Attrs.class_("text-xs text-gray-500 mb-2")}, list{text("TypeLL provides type intelligence to every panel, not just this one.")}),
                    div(
                      ...
                      list{
                        div(list{Attrs.class_("divide-y divide-gray-800")}, list{
                          ...
                          div(list{Attrs.class_("flex p-2 text-xs")}, list{
                            ...
                            span(list{Attrs.class_("text-gray-400")}, list{text("Schema type compatibility, adapter type safety")}),
                          }),
                          div(list{Attrs.class_("flex p-2 text-xs")}, list{
                            span(list{Attrs.class_("w-40 text-cyan-400")}, list{text("My-Lang")}),
                            span(list{Attrs.class_("text-gray-400")}, list{text("Full type checking across Solo/Duet/Ensemble/Me")}),
                          }),
                          div(list{Attrs.class_("flex p-2 text-xs")}, list{
                            span(list{Attrs.class_("w-40 text-cyan-400")}, list{text("Anti-Crash")}),
                            span(list{Attrs.class_("text-gray-400")}, list{text("Type-level validation before token acceptance")}),
                          }),
                          div(list{Attrs.class_("flex p-2 text-xs")}, list{
                            span(list{Attrs.class_("w-40 text-cyan-400")}, list{text("Pane-L")}),
                            span(list{Attrs.class_("text-gray-400")}, list{text("Type constraints as first-class symbolic constraints")}),
                          }),
                          div(list{Attrs.class_("flex p-2 text-xs")}, list{
                            span(list{Attrs.class_("w-40 text-cyan-400")}, list{text("BoJ")}),
                            span(list{Attrs.class_("text-gray-400")}, list{text("Cartridge ABI type checking (Idris2 formal specs)")}),
                          }),
                          div(list{Attrs.class_("flex p-2 text-xs")}, list{
                            span(list{Attrs.class_("w-40 text-cyan-400")}, list{text("ECHIDNA")}),
                            span(list{Attrs.class_("text-gray-400")}, list{text("Proof obligation dispatch — TypeLL generates, ECHIDNA proves")}),
                          }),
                        }),
                      },
                    ),
          ...
          // Loading/error
          if tl.loading {
            div(list{Attrs.class_("mt-4 text-gray-400 text-sm"), Attrs.role("status")}, list{text("Processing...")})
          } else { noNode },
          switch tl.error {
