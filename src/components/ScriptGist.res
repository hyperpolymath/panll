...
let renderTab = (label: string, cat: gistCategory, active: gistCategory, count: int): Tea_Vdom.t<
  msg,
> => {
      ...
      div(
        ...
        list{
          div(
            ...
            list{
              span(list{Attrs.class_("text-xs font-medium text-gray-200")}, list{text(gist.title)}),
              span(
                list{
                  Attrs.class_(
                    "text-[10px] px-1.5 py-0.5 rounded " ++
                    ScriptGistEngine.languageColour(gist.language) ++ " bg-gray-900/60",
                  ),
                },
      ...
      div(
        ...
        list{
          ...
          div(
            ...
            list{
              input(
                list{
                  Attrs.class_(
                    "flex-1 bg-gray-800 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 focus:border-cyan-500 outline-none",
                  ),
              ...
              button(
                list{
                  Attrs.class_(
                    "px-2 py-1.5 text-xs bg-cyan-700 hover:bg-cyan-600 text-white rounded",
                  ),
                  Events.onClick(ScriptGist(ExecuteGist)),
                  KeyboardNav.onActivate(ScriptGist(ExecuteGist)),
                },
                list{text("Run")},
              ),
              button(
                list{
                  Attrs.class_(
                    "px-2 py-1.5 text-xs bg-gray-700 hover:bg-gray-600 text-gray-300 rounded",
                  ),
          ...
          div(
            ...
            list{
              ...
              span(
                ...
                list{
                  text(
                    "~" ++ Int.toString(ScriptGistEngine.schemaTokenCost(gist.schema)) ++ " tokens",
                  ),
                },
              ),
          ...
          textarea(
            list{
              Attrs.class_(
                "w-full flex-1 min-h-[300px] bg-gray-900 border border-gray-700 rounded p-3 text-xs text-gray-200 font-mono resize-none focus:border-cyan-500 outline-none",
              ),
    ...
    div(
      ...
      ->Array.map(cp =>
        button(
          list{
            Attrs.class_(
              "w-full text-left px-2 py-1 text-[10px] text-gray-400 hover:bg-gray-800 rounded",
            ),
  ...
  div(
    ...
    ->Array.map(tpl =>
      button(
        list{
          Attrs.class_(
            "p-3 bg-gray-800/60 border border-gray-700 rounded hover:bg-gray-800 text-left transition-colors",
          ),
          Events.onClick(ScriptGist(CreateFromTemplate(tpl.id))),
        },
        list{
          div(list{Attrs.class_("text-xs font-medium text-gray-200 mb-1")}, list{text(tpl.name)}),
          div(list{Attrs.class_("text-[10px] text-gray-500")}, list{text(tpl.description)}),
          div(
            list{
              Attrs.class_("text-[10px] mt-1 " ++ ScriptGistEngine.languageColour(tpl.language)),
            },
  ...
  div(
    ...
    list{
          ...
          div(
            ...
            list{
              span(
                ...
                list{text("Script Gist")},
              ),
              span(
                list{Attrs.class_("text-[10px] text-gray-600")},
                list{text("Minskian Drafting Board")},
              ),
            },
          ),
          div(
            ...
            list{
              button(
                list{
                  Attrs.class_(
                    "px-2 py-1 text-xs bg-cyan-700 hover:bg-cyan-600 text-white rounded",
                  ),
                  Events.onClick(ScriptGist(CreateGist)),
                  KeyboardNav.onActivate(ScriptGist(CreateGist)),
                },
                list{text("+ New Gist")},
              ),
              button(
                list{
                  Attrs.class_(
                    "px-2 py-1 text-xs rounded " ++ (
                      state.mcpToolsActive
                        ? "bg-green-700 text-white"
                        : "bg-gray-700 text-gray-400 hover:bg-gray-600"
                    ),
                  ),
                  Events.onClick(ScriptGist(ToggleMcpTools)),
                  KeyboardNav.onActivate(ScriptGist(ToggleMcpTools)),
                },
      ...
      div(
        ...
        ->Array.map(cat =>
          renderTab(
            ScriptGistEngine.categoryLabel(cat),
            cat,
            state.activeCategory,
            ScriptGistEngine.countByCategory(state.gists, cat),
          )
        )
        ->List.fromArray,
      ),
      ...
      | Some(err) =>
        div(
          list{
            Attrs.class_(
              "mx-4 mt-2 p-2 bg-red-900/30 border border-red-800 rounded text-xs text-red-400 flex justify-between",
            ),
          },
          list{
            ...
            button(
              list{
                Attrs.class_("text-red-500 hover:text-red-300"),
                Events.onClick(ScriptGist(DismissGistError)),
                KeyboardNav.onActivate(ScriptGist(DismissGistError)),
              },
              list{text("dismiss")},
            ),
          },
            ...
            list{
              ...
              div(
                ...
                list{
                  input(
                    list{
                      Attrs.class_(
                        "w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs text-gray-300 placeholder-gray-600 outline-none focus:border-cyan-600",
                      ),
              ...
              div(
                ...
                if state.activeCategory === GistTemplates {
                  ...
                  filtered
                  ->Array.map(g => renderGistRow(g, Some(g.id) === state.selectedGistId))
                  ->List.fromArray
                },
              ),
            },
          ...
          div(
            ...
            list{
              div(
                ...
                list{
                  span(
                    list{Attrs.class_("text-[10px] text-gray-500 uppercase tracking-wide")},
                    list{text("Diachronic (Time)")},
                  ),
                },
              ),
              div(
              ...
              div(
                ...
                list{
                  span(
                    list{Attrs.class_("text-[10px] text-gray-500 uppercase tracking-wide")},
                    list{text("Synchronic (Space)")},
                  ),
                },
              ),
              div(list{Attrs.class_("flex-1 overflow-y-auto p-1")}, list{renderCardfiles(state)}),
            },
          ),
        },
      ),
