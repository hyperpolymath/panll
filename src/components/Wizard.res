...
let tierToString = (tier: ProvisionerModel.pluginTrustTier): string => {
  switch tier {
  | Teranga => "Teranga"
  | Shield => "Shield"
  | Ayo => "Ayo"
  ...
  div(list{}, list{
    ...
    div(list{Attrs.class_("grid grid-cols-2 gap-4")}, list{
      ...
      button(
        list{
          Attrs.class_(
            `p-6 border-2 rounded-lg transition-all ${state.creationType === Some(WizardModel.CreatingPanel) ? "border-indigo-500 bg-indigo-900" : "border-gray-700 hover:border-gray-600"}`
          ),
      ...
      button(
        list{
          Attrs.class_(
            `p-6 border-2 rounded-lg transition-all ${state.creationType === Some(WizardModel.CreatingPlugin) ? "border-indigo-500 bg-indigo-900" : "border-gray-700 hover:border-gray-600"}`
          ),
    ...
    List.concat(
      list{
        ...
        if requiredDeps->Array.length > 0 {
          div(list{Attrs.class_("p-3 bg-yellow-900/30 rounded-lg")}, list{
            ...
            ul(
              list{Attrs.class_("list-disc list-inside mt-2 text-yellow-100 text-sm")},
              requiredDeps->Array.map(depId => li(list{}, list{text(depId)}))->List.fromArray
            ),
          })
        } else {
          noNode
      ...
      categories->Array.map(category => {
        ...
        div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
          ...
          div(
            list{Attrs.class_("grid grid-cols-1 md:grid-cols-2 gap-3")},
            categoryCaps->Array.map(cap => {
              ...
              button(
                list{
                  Attrs.class_(
                    `p-3 border rounded transition-all flex flex-col ${isSelected ? "border-indigo-500 bg-indigo-900/30" : "border-gray-600 hover:border-gray-500"}`
                  ),
                  Events.onClick(Wizard(ToggleCapability(cap.id))),
                },
                list{
                  div(list{Attrs.class_("flex justify-between items-start")}, list{
                    ...
                    if hasDeps {
                      ...
                      }, list{text(`${cap.requiredDependencies->Array.length->Int.toString} deps`)})
                    } else {
                      noNode
                    }
                  }),
    ...
    div(list{Attrs.class_("flex gap-4")}, list{
      div(list{Attrs.class_("flex-1")}, list{
        div(list{Attrs.class_("p-4 border border-gray-700 rounded-lg")}, list{
          ...
          p(list{Attrs.class_("text-xs text-gray-400 mb-3")}, list{
            text(`${requiredDeps->Array.length->Int.toString} dependencies needed`)
          }),
          if requiredDeps->Array.length > 0 {
            ul(
              list{Attrs.class_("space-y-2 text-sm")},
      ...
      div(list{Attrs.class_("flex-1")}, list{
        div(list{Attrs.class_("p-4 border border-gray-700 rounded-lg")}, list{
          ...
          p(list{Attrs.class_("text-xs text-gray-400 mb-3")}, list{
            text(`${state.dependencies->Array.length->Int.toString} dependencies configured`)
          }),
                    ...
                    td(list{Attrs.class_("py-2")}, list{
                      input(
                        list{
                          ...
                          Events.onInput(newVersion => Wizard(AddDependency(dep.pluginId, newVersion))),
                        },
                        list{}
                      )
                    }),
                    td(list{Attrs.class_("py-2")}, list{
                      select(
                        list{
                          ...
                          Attrs.value(tierToString(dep.tier)),
                        },
                        list{
        ...
        div(list{Attrs.class_("space-y-3")}, list{
          ...
          div(list{}, list{
            ...
            select(
              ...
              List.concat(
                list{option'(list{Attrs.value("")}, list{text("Select plugin...")})},
                requiredDeps->Array.map(depId =>
          ...
          div(list{}, list{
            ...
            select(
              list{Attrs.class_("w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded text-sm")},
              list{
...
let renderSetupSecurity = (state: WizardModel.wizardState): Tea_Vdom.t<msg> => {
  div(list{Attrs.class_("space-y-6")}, list{
    ...
    div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
      ...
      div(list{Attrs.class_("space-y-3")}, list{
        ...
        button(
          list{
            Attrs.class_(
              `w-full p-4 border-2 rounded-lg text-left transition-all ${state.securityConfig.trustTier === Teranga ? "border-yellow-500 bg-yellow-900/30" : "border-gray-600 hover:border-gray-500"}`
            ),
            Events.onClick(Wizard(SetTrustTier(Teranga))),
          },
          list{
        ...
        button(
          list{
            Attrs.class_(
              `w-full p-4 border-2 rounded-lg text-left transition-all ${state.securityConfig.trustTier === Shield ? "border-red-500 bg-red-900/30" : "border-gray-600 hover:border-gray-500"}`
            ),
            Events.onClick(Wizard(SetTrustTier(Shield))),
          },
          list{
            div(list{Attrs.class_("flex justify-between items-center")}, list{
              div(list{}, list{
        ...
        button(
          list{
            Attrs.class_(
              `w-full p-4 border-2 rounded-lg text-left transition-all ${state.securityConfig.trustTier === Ayo ? "border-blue-500 bg-blue-900/30" : "border-gray-600 hover:border-gray-500"}`
            ),
            Events.onClick(Wizard(SetTrustTier(Ayo))),
          },
          list{
            div(list{Attrs.class_("flex justify-between items-center")}, list{
              div(list{}, list{
      ...
      div(list{Attrs.class_("space-y-4")}, list{
        ...
        div(list{Attrs.class_("flex items-center justify-between p-3 border border-gray-700 rounded")}, list{
          div(list{}, list{
            ...
            p(list{Attrs.class_("text-xs text-gray-400")}, list{text("Allow outgoing network requests")}),
          }),
          label(list{Attrs.class_("relative inline-flex items-center cursor-pointer")}, list{
            input(list{
              ...
              Events.onChange(_ => Wizard(ToggleNetworkAccess(!state.securityConfig.networkAccess))),
            }, list{}),
            div(list{Attrs.class_("w-11 h-6 bg-gray-600 rounded-full peer peer-checked:bg-indigo-600")}, list{}),
          })
        }),
        ...
        div(list{Attrs.class_("flex items-center justify-between p-3 border border-gray-700 rounded")}, list{
          div(list{}, list{
            ...
            p(list{Attrs.class_("text-xs text-gray-400")}, list{text("Allow filesystem read/write operations")}),
          }),
          label(list{Attrs.class_("relative inline-flex items-center cursor-pointer")}, list{
            input(list{
              Attrs.type_("checkbox"),
              ...
              Events.onChange(_ => Wizard(ToggleFilesystemAccess(!state.securityConfig.filesystemAccess))),
            }, list{}),
            div(list{Attrs.class_("w-11 h-6 bg-gray-600 rounded-full peer peer-checked:bg-indigo-600")}, list{}),
          })
        }),
    ...
    // Capability restrictions placeholder
    div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
...
let renderReviewAndGenerate = (state: WizardModel.wizardState): Tea_Vdom.t<msg> => {
  ...
  let isPanel = state.creationType === Some(WizardModel.CreatingPanel)
  ...
  div(list{Attrs.class_("space-y-6")}, list{
    ...
    div(list{Attrs.class_("grid grid-cols-1 md:grid-cols-3 gap-4")}, list{
      ...
      div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
        ...
        p(list{Attrs.class_("text-xs text-gray-400 mb-2")}, list{text(isPanel ? "Panel" : "Plugin")}),
        p(list{Attrs.class_("text-xs text-gray-500")}, list{text(isPanel ? "UI component" : "Backend cartridge")}),
      }),
      ...
      div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
        ...
        p(list{Attrs.class_("text-xs text-gray-400 mb-2")}, list{
          text(`${capabilityCount->Int.toString} capabilities selected`)
        }),
        if capabilityCount > 0 {
          div(list{}, list{
            ul(
              list{Attrs.class_("text-xs text-gray-500 mt-2 space-y-1 list-disc list-inside")},
              state.selectedCapabilities
              ->Array.slice(~start=0, ~end=3)
              ->Array.map(capId => {
                let name = WizardModel.capabilityRegistry
                  ->Array.find(c => c.id === capId)
                  ->Option.mapOr(capId, c => c.name)
                li(list{}, list{text(name)})
              })
              ->List.fromArray
            ),
            if capabilityCount > 3 {
              p(list{Attrs.class_("text-xs text-gray-500 mt-1")}, list{
                text(`... and ${Int.toString(capabilityCount - 3)} more`)
              })
            } else {
              noNode
            }
      ...
      div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
        ...
        p(list{Attrs.class_("text-xs text-gray-400 mb-2")}, list{
          text(`${dependencyCount->Int.toString} dependencies configured`)
        }),
        if dependencyCount > 0 {
          div(list{}, list{
            ul(
              ...
              state.dependencies
              ->Array.slice(~start=0, ~end=3)
              ->Array.map(dep => li(list{}, list{text(`${dep.pluginId} v${dep.version}`)}) )
              ->List.fromArray
            ),
            if dependencyCount > 3 {
              p(list{Attrs.class_("text-xs text-gray-500 mt-1")}, list{
                text(`... and ${Int.toString(dependencyCount - 3)} more`)
              })
            } else {
              noNode
            }
      ...
      div(list{Attrs.class_("space-y-3")}, list{
        div(list{}, list{
          ...
          p(list{Attrs.class_("font-medium text-gray-200")}, list{
            text(switch state.securityConfig.trustTier {
    ...
    div(list{Attrs.class_("border border-gray-700 rounded-lg p-4")}, list{
      ...
      if validationErrors->Array.length === 0 {
        ...
        div(list{}, list{
          ...
          ul(
            list{Attrs.class_("text-red-300 text-sm mt-2 list-disc list-inside")},
            validationErrors->Array.map(error => li(list{}, list{text(error)}))->List.fromArray
          ),
        })
      }
    }),
    ...
    button(
      list{
        ...
        Events.onClick(Wizard(StartGeneration)),
      },
      list{
        text(state.generating ? "Generating..." : "Generate Component")
      }
    ),
  })
}
...
let renderWizard = (state: model): Tea_Vdom.t<msg> => {
  ...
  div(
    list{Attrs.class_("wizard-container p-6 bg-gray-900 rounded-lg max-w-2xl mx-auto")},
    list{
      // Header
      ...
      // Step indicator
      div(
        list{Attrs.class_("flex gap-4 mb-6")},
        [
          ...
          div(
            ...
            list{
              div(list{Attrs.class_("w-6 h-6 rounded-full border-2 flex items-center justify-center text-xs")}, list{
                text(isCompleted ? "✓" : Int.toString(WizardModel.stepIndex(step) + 1))
              }),
