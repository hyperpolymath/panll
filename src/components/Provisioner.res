...
let renderPortfolioCard = (
  ...
  let allInstalled = portfolio.panels->Array.every(p =>
    ProvisionerEngine.getInstallStatus(installStatuses, p) === Installed
  )
  let installedCount = portfolio.panels->Array.filter(p =>
    ProvisionerEngine.getInstallStatus(installStatuses, p) === Installed
  )->Array.length
    ...
    list{
      ...
      div(list{Attrs.class_("flex items-center justify-between mb-2")}, list{
        div(list{Attrs.class_("flex items-center gap-2")}, list{
          span(list{Attrs.class_("text-lg text-gray-200 font-medium")}, list{text(portfolio.name)}),
          if portfolio.builtIn {
            span(list{Attrs.class_("text-xs bg-indigo-900/50 text-indigo-400 px-1.5 py-0.5 rounded")}, list{text("Built-in")})
          } else {
            noNode
          },
        }),
        if allInstalled {
          ...
          button(
            list{
              Attrs.class_("px-3 py-1 text-xs bg-indigo-600 text-white rounded hover:bg-indigo-500 transition-colors"),
      ...
      // Panel chips
      div(list{Attrs.class_("flex flex-wrap gap-1.5 mb-2")},
        portfolio.panels->Array.map(panelName => {
      ...
      // Footer
      div(list{Attrs.class_("flex items-center justify-between text-xs text-gray-600")}, list{
        span(list{}, list{text(`${Int.toString(installedCount)}/${Int.toString(Array.length(portfolio.panels))} panels`)}),
...
let renderConfigRow = (config: panelConfig): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      // Panel name
      span(list{Attrs.class_("text-sm text-gray-200 w-28 font-medium")}, list{text(config.panelName)}),
      // Isolation tier
      span(list{Attrs.class_(`text-xs ${tierColour} w-20`)}, list{
        text(ProvisionerEngine.isolationShortLabel(config.isolation)),
      }),
      // Endpoint
      span(list{Attrs.class_("text-xs text-gray-500 flex-1 truncate")}, list{
        text(config.endpoint === "" ? "No endpoint" : config.endpoint),
      }),
      // Auto-connect indicator
      span(list{Attrs.class_(`text-xs ${config.autoConnect ? "text-green-400" : "text-gray-600"}`)}, list{
        text(config.autoConnect ? "Auto" : "Manual"),
      }),
      // Enabled toggle
      button(
        list{
      ...
      div(list{Attrs.class_("flex gap-1")}, list{
        button(
          list{
            Attrs.class_(`px-1.5 py-0.5 text-xs rounded ${config.isolation === Native ? "bg-green-900 text-green-400" : "bg-gray-800 text-gray-500"}`),
        ...
        button(
          list{
            Attrs.class_(`px-1.5 py-0.5 text-xs rounded ${config.isolation === StandardPod ? "bg-blue-900 text-blue-400" : "bg-gray-800 text-gray-500"}`),
        ...
        button(
          list{
            Attrs.class_(`px-1.5 py-0.5 text-xs rounded ${config.isolation === HardenedPod ? "bg-purple-900 text-purple-400" : "bg-gray-800 text-gray-500"}`),
...
let renderIsolationSummary = (configs: array<panelConfig>): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      span(list{Attrs.class_("text-xs text-green-400")}, list{
        text(`Native: ${Int.toString(native)}`),
      }),
      span(list{Attrs.class_("text-xs text-blue-400")}, list{
        text(`Standard Pod: ${Int.toString(standard)}`),
      }),
      span(list{Attrs.class_("text-xs text-purple-400")}, list{
        text(`Hardened Pod: ${Int.toString(hardened)}`),
      }),
      span(list{Attrs.class_("text-xs text-gray-500 ml-auto")}, list{
        text(`${Int.toString(native + standard + hardened)} total`),
      }),
    },
  )
}
    ...
    list{
      ...
      div(list{Attrs.class_("flex items-center justify-between p-4 border-b border-gray-800")}, list{
        div(list{Attrs.class_("flex items-center gap-3")}, list{
          h2(list{Attrs.class_("text-lg font-medium text-gray-200")}, list{text("Provisioner")}),
          span(list{Attrs.class_("text-xs text-gray-500")}, list{
            text(`${Int.toString(ProvisionerEngine.countInstalled(prov.installStatus))} panels installed`),
          }),
        }),
        button(
          list{
      ...
      div(list{Attrs.class_("flex-1 overflow-auto p-6")}, list{
        ...
        | Portfolios => {
            ...
            div(list{Attrs.class_("space-y-4")}, list{
              ...
              input(
                list{
                  Attrs.class_("w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200 mb-4"),
              ...
              div(list{Attrs.class_("grid gap-4 grid-cols-1 lg:grid-cols-2")},
                filtered->Array.map(p => renderPortfolioCard(p, prov.installStatus))->List.fromArray,
              ),
            })
          }
        | Configurator =>
          div(list{Attrs.class_("space-y-2")}, list{
            ...
            // Column headers
            div(list{Attrs.class_("flex items-center gap-3 px-3 py-1 text-xs text-gray-600")}, list{
        ...
        | Installed => {
            ...
            div(list{Attrs.class_("space-y-2")}, list{
              renderIsolationSummary(prov.configs),
              div(list{Attrs.class_("space-y-1")},
                installed->Array.map(((name, _status)) => {
                  ...
                  div(
                    ...
                    list{
                      span(list{Attrs.class_("text-sm text-gray-200 w-28 font-medium")}, list{text(name)}),
                      span(list{Attrs.class_(`text-xs ${tierColour}`)}, list{text(tierLabel)}),
                      span(list{Attrs.class_("text-xs text-green-400 ml-auto")}, list{text("Running")}),
                      button(
                        list{
                          Attrs.class_("px-2 py-0.5 text-xs bg-red-900/50 text-red-400 rounded hover:bg-red-900"),
        ...
        | CustomPortfolio =>
          div(list{Attrs.class_("space-y-4")}, list{
            div(list{Attrs.class_("text-sm text-gray-400 mb-2")}, list{
              text("Build a custom portfolio by selecting panels:"),
            }),
            input(
              list{
                Attrs.class_("w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm text-gray-200"),
            ...
            div(list{Attrs.class_("flex flex-wrap gap-2 mt-3")},
              prov.configs->Array.map(c => {
            ...
            if Array.length(prov.customPanels) > 0 {
              div(list{Attrs.class_("mt-4")}, list{
                button(
                  list{
                    Attrs.class_("px-4 py-2 text-sm bg-indigo-600 text-white rounded hover:bg-indigo-500"),
                    Events.onClick(Provisioner(SaveCustomPortfolio)),
                  },
                  list{text(`Save Portfolio (${Int.toString(Array.length(prov.customPanels))} panels)`)},
                ),
              })
            } else {
              noNode
