...
let presenceIcon = (present: bool): Tea_Vdom.t<msg> => {
  let (color, label) = if present {("text-green-400", "Y")} else {("text-red-400", "N")}
  ...
  button(
    ...
    list{
      div(list{Attrs.class_("flex items-center justify-between")}, list{
        ...
        span(
          list{Attrs.class_(if complete {"text-xs text-green-400"} else {"text-xs text-amber-400"})},
          list{text(if complete {"Complete"} else {"Incomplete"})},
        ),
      }),
    },
  )
...
/// Render a tab button.
let tabBtn = (current: contractileCompletenessTab, target: contractileCompletenessTab, label: string): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      ...
      div(
        ...
        list{
          div(list{Attrs.class_("flex items-center gap-3")}, list{
            h2(list{Attrs.class_("text-lg font-bold text-amber-300")}, list{text("Contractile Completeness")}),
            span(list{Attrs.class_("text-xs text-gray-500")}, list{
              text(`${Int.toString(complete)}/${Int.toString(total)} complete`),
            }),
          }),
          button(
            list{
      ...
      div(
        ...
        ContractileCompletenessEngine.allTabs->Array.map(t =>
          tabBtn(state.activeTab, t, ContractileCompletenessEngine.tabLabel(t))
        )->List.fromArray,
      ),
      // Error banner
      switch state.error {
                  ...
                  div(list{}, list{
                    ...
                    div(list{Attrs.class_("space-y-2")}, list{
                      div(list{Attrs.class_("flex items-center gap-2")}, list{
                        span(list{Attrs.class_("text-xs text-gray-500 w-24")}, list{text("Mustfile:")}),
                        presenceIcon(repo.hasMustfile),
                      }),
                      div(list{Attrs.class_("flex items-center gap-2")}, list{
                        span(list{Attrs.class_("text-xs text-gray-500 w-24")}, list{text("Trustfile:")}),
                        presenceIcon(repo.hasTrustfile),
                      }),
                      div(list{Attrs.class_("flex items-center gap-2")}, list{
                        span(list{Attrs.class_("text-xs text-gray-500 w-24")}, list{text("Dustfile:")}),
                        presenceIcon(repo.hasDustfile),
                      }),
                      div(list{Attrs.class_("flex items-center gap-2")}, list{
                        span(list{Attrs.class_("text-xs text-gray-500 w-24")}, list{text("K9:")}),
                        presenceIcon(repo.hasK9),
                        if repo.hasK9 {
                          span(list{Attrs.class_("text-xs text-gray-500 ml-2")}, list{
                            text(`(${Int.toString(repo.k9Count)} configs)`),
                          })
                        } else {
                          noNode
                        },
      ...
      div(
        ...
        list{text(`${Int.toString(ContractileCompletenessEngine.incompleteCount(state.repos))} repos incomplete`)},
      ),
    },
  )
}
