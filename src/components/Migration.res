...
let renderRepoRow = (repo: migrationRepoSummary): Tea_Vdom.t<msg> => {
  ...
  div(
    list{Attrs.class_("flex items-center gap-3 p-2 border-b border-gray-800 hover:bg-gray-900/50"), Attrs.role("row")},
    list{
      // Blocked indicator
      if repo.blocked {
      ...
      // Repo name
      span(list{Attrs.class_("text-sm text-gray-200 w-40 truncate font-medium")}, list{text(repo.name)}),
      ...
      // Health bar
      div(list{Attrs.class_("flex-1 flex items-center gap-2")}, list{
        renderHealthBar(repo.healthScore),
        span(
          list{Attrs.class_(`text-xs w-10 text-right ${MigrationEngine.healthColor(repo.healthScore)}`)},
          list{text(MigrationEngine.healthPercent(repo.healthScore))},
        ),
      }),
      // Trend
      span(list{Attrs.class_(`text-xs w-12 ${trendClass}`)}, list{text(MigrationEngine.trendLabel(repo.trend))}),
      // Deprecated / modern counts
      span(list{Attrs.class_("text-xs text-red-400 w-16 text-right")}, list{text(`${Int.toString(repo.deprecatedCount)} dep`)}),
      span(list{Attrs.class_("text-xs text-green-400 w-16 text-right")}, list{text(`${Int.toString(repo.modernCount)} mod`)}),
      // Config format
      span(list{Attrs.class_("text-xs text-gray-500 w-24 text-right")}, list{text(MigrationEngine.configFormatLabel(repo.configFormat))}),
    },
  )
}
        ...
        groups->Array.map(((bracket, rs)) => {
          ...
          div(
            list{
              ...
              Attrs.prop("style", `width: ${pct}%`),
              Attrs.title(`${MigrationEngine.versionBracketLabel(bracket)}: ${Int.toString(count)} repos`),
            },
            list{
              if count > 2 {
                span(list{Attrs.class_("text-xs text-gray-900 font-medium")}, list{text(Int.toString(count))})
              } else {
                noNode
              },
            },
      ...
      div(
        ...
        groups->Array.map(((bracket, rs)) => {
          ...
          div(list{Attrs.class_("flex items-center gap-1")}, list{
            ...
            span(list{Attrs.class_("text-xs text-gray-400")}, list{text(`${MigrationEngine.versionBracketLabel(bracket)} (${Int.toString(Array.length(rs))})`)})
          })
        })->List.fromArray,
      ),
    })
...
let renderConstraintStrip = (constraints: array<migrationConstraint>): Tea_Vdom.t<msg> => {
  ...
  if count === 0 {
    div(list{Attrs.class_("text-xs text-green-400 px-3 py-1 bg-green-900/20 border border-green-800 rounded")}, list{
      text("All migration constraints satisfied"),
    })
  } else {
    div(list{Attrs.class_("flex flex-wrap gap-2")}, list{
      span(list{Attrs.class_("text-xs text-red-400 font-medium")}, list{text(`${Int.toString(count)} constraints remaining:`)}),
      ...unsatisfied->Array.map(c =>
        span(
          list{Attrs.class_("text-xs px-2 py-0.5 bg-red-900/30 border border-red-800 rounded")},
          list{text(`${c.pattern} (${Int.toString(c.totalCount)} in ${Int.toString(c.repoCount)} repos)`)},
        )
      )->List.fromArray,
    })
  }
...
let renderObligations = (obligations: array<migrationObligation>): Tea_Vdom.t<msg> => {
  if Array.length(obligations) === 0 {
    ...
    div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-4")}, list{
      div(list{Attrs.class_("flex items-center gap-2 mb-3")}, list{
        span(list{Attrs.class_("text-sm font-medium text-gray-300")}, list{text("Proof Obligations")}),
        span(list{Attrs.class_("text-xs text-gray-500")}, list{text(`${Int.toString(met)}/${Int.toString(total)} met`)}),
      }),
      div(list{Attrs.class_("space-y-1")},
        obligations->Array.map(o => {
          let icon = o.met ? "text-green-400" : "text-red-400"
          let mark = o.met ? "[OK]" : "[!!]"
          div(list{Attrs.class_("flex items-center gap-2 text-xs")}, list{
...
let renderSessionRow = (session: migrationSession): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      ...
      div(list{Attrs.class_("flex items-center gap-2 flex-1")}, list{
        span(list{Attrs.class_("text-xs text-gray-400")}, list{text(`Before: ${MigrationEngine.healthPercent(session.beforeHealth)}`)}),
        switch session.afterHealth {
        | Some(after) =>
          span(list{Attrs.class_("text-xs text-gray-400")}, list{text(`After: ${MigrationEngine.healthPercent(after)}`)})
        | None => noNode
        },
        ...
        | Some(delta) =>
          ...
          let sign = delta > 0.0 ? "+" : ""
          span(list{Attrs.class_(`text-xs font-medium ${deltaClass}`)}, list{text(`${sign}${Float.toFixed(delta *. 100.0, ~digits=1)}%`)})
        | None => noNode
        },
      }),
      span(list{Attrs.class_("text-xs text-gray-500 w-20 text-right")}, list{text(`${Int.toString(session.issueCount)} issues`)}),
      span(list{Attrs.class_("text-xs text-gray-600 w-28 text-right")}, list{text(session.startedAt)}),
    },
  )
}
...
let renderSubmissionRow = (sub: migrationSubmission): Tea_Vdom.t<msg> => {
  ...
  div(
    list{Attrs.class_("flex items-center gap-3 p-3 border-b border-gray-800")},
    list{
      span(list{Attrs.class_(`text-xs ${statusColor} w-16`)}, list{text(MigrationEngine.submissionStatusLabel(sub.status))}),
      div(list{Attrs.class_("flex-1")}, list{
        div(list{Attrs.class_("text-sm text-gray-200")}, list{text(sub.title)}),
        div(list{Attrs.class_("text-xs text-gray-500")}, list{text(`${sub.repo} | ${sub.severity}`)}),
      }),
      if sub.status == SubmissionPending {
        div(list{Attrs.class_("flex gap-2")}, list{
          button(
            list{Attrs.class_("px-2 py-1 text-xs bg-green-700 text-white rounded hover:bg-green-600"), Events.onClick(Migration(ApproveSubmission(sub.id)))},
            list{text("Approve")},
          ),
          button(
            list{Attrs.class_("px-2 py-1 text-xs bg-red-700 text-white rounded hover:bg-red-600"), Events.onClick(Migration(RejectSubmission(sub.id)))},
            list{text("Reject")},
          ),
        })
      } else {
...
let renderMergeRow = (merge: mergeResolution): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    list{
      ...
      span(list{Attrs.class_("text-xs text-gray-500 w-40")}, list{text(`${merge.sourceBranch} -> ${merge.targetBranch}`)}),
      span(list{Attrs.class_("text-xs text-gray-400 w-24")}, list{text(`${Int.toString(merge.resolvedCount)}/${Int.toString(merge.conflictCount)} resolved`)}),
      span(list{Attrs.class_(`text-xs w-16 ${confColor}`)}, list{text(`${Float.toFixed(merge.avgConfidence *. 100.0, ~digits=0)}% conf`)}),
      if merge.status === "in_progress" {
        div(list{Attrs.class_("flex gap-2")}, list{
          button(
            list{Attrs.class_("px-2 py-1 text-xs bg-green-700 text-white rounded hover:bg-green-600"), Events.onClick(Migration(AcceptMerge(merge.sessionId)))},
            list{text("Accept")},
          ),
          button(
            list{Attrs.class_("px-2 py-1 text-xs bg-red-700 text-white rounded hover:bg-red-600"), Events.onClick(Migration(RollbackMerge(merge.sessionId)))},
            list{text("Rollback")},
          ),
        })
      } else {
      ...
      span(list{Attrs.class_("text-xs text-gray-600 w-28 text-right")}, list{text(merge.timestamp)}),
    },
  )
}
...
let renderTabs = (active: migrationCategory): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    tabs->Array.map(tab => {
      ...
      button(
        list{
          Attrs.class_(`px-4 py-2 text-sm rounded-t transition-colors ${isActive ? "bg-gray-800 text-gray-200 border-b-2 border-indigo-500" : "text-gray-500 hover:text-gray-300"}`),
          Attrs.role("tab"),
...
let renderReportTypeSelector = (active: migrationReportType): Tea_Vdom.t<msg> => {
  ...
  div(
    ...
    types->Array.map(rt => {
      ...
      button(
        list{
          Attrs.class_(`px-3 py-1 text-xs rounded transition-colors ${isActive ? "bg-indigo-600 text-white" : "bg-gray-800 text-gray-400 hover:text-gray-200"}`),
...
let renderDashboard = (mig: migrationState): Tea_Vdom.t<msg> => {
  ...
  div(list{Attrs.class_("space-y-6")}, list{
    // Stat cards
    div(list{Attrs.class_("flex gap-4 text-sm")}, list{
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3 flex-1")}, list{
        div(list{Attrs.class_("text-gray-500 text-xs uppercase tracking-wider mb-1")}, list{text("Repos")}),
        div(list{Attrs.class_("text-2xl font-light text-gray-200")}, list{text(Int.toString(mig.totalRepos))}),
      }),
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3 flex-1")}, list{
        div(list{Attrs.class_("text-gray-500 text-xs uppercase tracking-wider mb-1")}, list{text("Avg Health")}),
        div(list{Attrs.class_(`text-2xl font-light ${MigrationEngine.healthColor(mig.avgHealth)}`)}, list{text(MigrationEngine.healthPercent(mig.avgHealth))}),
      }),
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3 flex-1")}, list{
        div(list{Attrs.class_("text-gray-500 text-xs uppercase tracking-wider mb-1")}, list{text("Ready")}),
        div(list{Attrs.class_("text-2xl font-light text-green-400")}, list{text(Int.toString(mig.readyCount))}),
      }),
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3 flex-1")}, list{
        div(list{Attrs.class_("text-gray-500 text-xs uppercase tracking-wider mb-1")}, list{text("Blocked")}),
        div(list{Attrs.class_("text-2xl font-light text-red-400")}, list{text(Int.toString(mig.blockedCount))}),
      }),
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-3 flex-1")}, list{
        div(list{Attrs.class_("text-gray-500 text-xs uppercase tracking-wider mb-1")}, list{text("Velocity")}),
        div(list{Attrs.class_("text-2xl font-light text-indigo-400")}, list{text(`${Float.toFixed(mig.velocity *. 100.0, ~digits=1)}%/sess`)}),
      }),
    }),
    ...
    // Repo table
    div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg overflow-hidden")}, list{
      div(list{Attrs.class_("flex items-center gap-3 p-3 border-b border-gray-700 text-xs text-gray-500 uppercase tracking-wider")}, list{
...
let renderTimeline = (mig: migrationState): Tea_Vdom.t<msg> => {
  div(list{Attrs.class_("space-y-4")}, list{
    div(list{Attrs.class_("flex items-center justify-between mb-2")}, list{
      div(list{Attrs.class_("text-sm text-gray-400")}, list{text(`${Int.toString(MigrationEngine.activeSessions(mig.sessions))} active sessions`)}),
    }),
    if Array.length(mig.sessions) === 0 {
      div(list{Attrs.class_("text-center text-gray-500 mt-8")}, list{
        ...
        div(list{Attrs.class_("text-sm")}, list{text("Use feedback-o-tron to begin a migration observation")}),
      })
    } else {
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg overflow-hidden")}, list{
        div(
...
let renderReports = (mig: migrationState): Tea_Vdom.t<msg> => {
  div(list{Attrs.class_("space-y-4")}, list{
    ...
    div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-6")}, list{
      switch mig.activeReportType {
      | PerRepoReport =>
        div(list{}, list{
          div(list{Attrs.class_("text-sm font-medium text-gray-300 mb-2")}, list{text("Per-Repository Reports")}),
          div(list{Attrs.class_("text-xs text-gray-500 mb-4")}, list{text("Before/after migration tables, issues, recommendations per repo")}),
          if Array.length(mig.repos) > 0 {
            div(list{Attrs.class_("space-y-2")},
              mig.repos->Array.map(r =>
                div(list{Attrs.class_("flex items-center gap-3 p-2 hover:bg-gray-800/50 rounded")}, list{
                  span(list{Attrs.class_("text-sm text-gray-300 w-40")}, list{text(r.name)}),
                  renderHealthBar(r.healthScore),
                  span(list{Attrs.class_(`text-xs ${MigrationEngine.healthColor(r.healthScore)}`)}, list{text(MigrationEngine.healthPercent(r.healthScore))}),
                  span(list{Attrs.class_("text-xs text-gray-500")}, list{text(`${Int.toString(r.deprecatedCount)} deprecated`)}),
                })
              )->List.fromArray,
            )
          } else {
            div(list{Attrs.class_("text-gray-500 text-sm")}, list{text("No repo data available. Run panic-attack migration-snapshot first.")})
          },
        })
      | CrossRepoReport =>
        div(list{}, list{
          div(list{Attrs.class_("text-sm font-medium text-gray-300 mb-2")}, list{text("Cross-Repository Aggregation")}),
          div(list{Attrs.class_("text-xs text-gray-500 mb-4")}, list{text("Common pain points, average health improvement, migration velocity")}),
          div(list{Attrs.class_("flex gap-4 text-xs text-gray-400")}, list{
            span(list{}, list{text(`${Int.toString(mig.totalRepos)} repos tracked`)}),
            span(list{}, list{text(`Avg health: ${MigrationEngine.healthPercent(mig.avgHealth)}`)}),
            span(list{}, list{text(`Velocity: ${Float.toFixed(mig.velocity *. 100.0, ~digits=1)}%/session`)}),
          }),
        })
      | V13TrialReport =>
        div(list{}, list{
          div(list{Attrs.class_("text-sm font-medium text-gray-300 mb-2")}, list{text("v13 Pre-Release Trial Report")}),
          div(list{Attrs.class_("text-xs text-gray-500 mb-4")}, list{text("Performance data, regressions, missing features for ReScript team")}),
          div(list{Attrs.class_("text-xs text-gray-500")}, list{text("Generate report after completing v13 migration sessions")}),
        })
      },
    }),
  })
...
let renderSubmissions = (mig: migrationState): Tea_Vdom.t<msg> => {
  ...
  div(list{Attrs.class_("space-y-4")}, list{
    div(list{Attrs.class_("flex items-center justify-between mb-2")}, list{
      div(list{Attrs.class_("text-sm text-gray-400")}, list{text(`${Int.toString(pending)} pending review`)}),
      if pending > 0 {
        button(
          list{
    ...
    if Array.length(mig.submissions) === 0 {
      div(list{Attrs.class_("text-center text-gray-500 mt-8")}, list{
        ...
        div(list{Attrs.class_("text-sm")}, list{text("Issues discovered during migration observations will appear here")}),
      })
    } else {
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg overflow-hidden")}, list{
        div(
...
let renderMergeResolver = (mig: migrationState): Tea_Vdom.t<msg> => {
  div(list{Attrs.class_("space-y-4")}, list{
    div(list{Attrs.class_("flex items-center justify-between mb-2")}, list{
      div(list{Attrs.class_("text-sm text-gray-400")}, list{text(`${Int.toString(Array.length(mig.mergeResolutions))} merge sessions`)}),
    }),
    if Array.length(mig.mergeResolutions) === 0 {
      div(list{Attrs.class_("text-center text-gray-500 mt-8")}, list{
        div(list{Attrs.class_("text-lg mb-2")}, list{text("No merge resolutions")}),
        div(list{Attrs.class_("text-sm mb-4")}, list{text("Use merge-resolver to begin resolving conflicts with rollback support")}),
        div(list{Attrs.class_("text-xs text-gray-600")}, list{text("merge-resolver begin <REPO> <BRANCH>")}),
      })
    } else {
      div(list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg overflow-hidden")}, list{
        div(
    ...
    list{
      ...
      div(list{Attrs.class_("flex items-center justify-between p-4 border-b border-gray-800")}, list{
        div(list{Attrs.class_("flex items-center gap-3")}, list{
          h2(list{Attrs.class_("text-lg font-medium text-gray-200")}, list{text("Migration Observatory")}),
          span(list{Attrs.class_("text-xs text-gray-500")}, list{text("ReScript migration health, sessions, submissions")}),
          if mig.loaded {
            span(list{Attrs.class_("text-xs text-indigo-400 ml-2")}, list{text(`${Int.toString(mig.totalRepos)} repos | ${MigrationEngine.healthPercent(mig.avgHealth)} avg`)})
          } else {
            noNode
          },
        }),
        div(list{Attrs.class_("flex items-center gap-3")}, list{
          ...
          if mig.loaded {
            input(
              list{
                Attrs.class_("px-3 py-1 text-xs bg-gray-800 border border-gray-700 rounded text-gray-300 w-48 placeholder-gray-600"),
      ...
      div(list{Attrs.class_("flex-1 overflow-auto p-6")}, list{
        if !mig.loaded {
          div(list{Attrs.class_("text-center text-gray-500 mt-12")}, list{
            ...
            div(list{Attrs.class_("text-sm mb-6")}, list{text("Track ReScript migration health across 54+ repos with before/after snapshots, session observation, issue submission, and merge conflict resolution")}),
            div(list{Attrs.class_("text-xs text-gray-600 mb-4")}, list{text("Data from panic-attack + feedback-o-tron + merge-resolver + Hypatia + VeriSimDB")}),
            button(
              list{
          ...
          div(list{Attrs.class_("text-center text-gray-400 mt-12"), Attrs.role("status")}, list{
            div(list{Attrs.class_("text-sm animate-pulse")}, list{text("Loading migration data...")}),
          })
        } else {
          div(list{Attrs.class_("space-y-4")}, list{
      ...
      | Some(e) =>
        div(
          ...
          list{
            span(list{Attrs.class_("text-xs text-red-400")}, list{text(e)}),
          },
        )
      | None => noNode
      },
