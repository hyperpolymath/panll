              ...
              list{
                  ...
                  div(
                    ...
                    list{
                      ...
                      div(
                        ...
                        ReposystemEngine.allRequirements->Array.map(req => {
                          let label = ReposystemEngine.requirementLabel(req)
                          let rate = ReposystemEngine.requirementRate(rsr.audits, req)
                          let pct = Float.toFixed(rate *. 100.0, ~digits=1)
                          let isSelected = rsr.selectedRequirement === Some(req)
                          let rateColor = if rate > 0.9 { "text-green-400" } else if rate > 0.5 { "text-amber-400" } else { "text-red-400" }
                          button(
                            list{
                              Attrs.class_(`w-full text-left p-3 rounded transition-colors flex items-center justify-between ${
                                isSelected ? "bg-cyan-900/40 border border-cyan-700" : "bg-gray-900 border border-gray-800 hover:bg-gray-800"
                              }`),
                            ...
                            list{
                              ...
                              span(list{Attrs.class_(`text-xs font-mono ${rateColor}`)}, list{text(`${pct}%`)}),
                            },
                          )
                        })->List.fromArray,
                      ),
                      ...
                      div(
                        ...
                        list{
                          ...
                          | Some(req) => {
                              let label = ReposystemEngine.requirementLabel(req)
                              let failing = ReposystemEngine.reposFailingRequirement(rsr.audits, req)
                              let passing = ReposystemEngine.reposPassingRequirement(rsr.audits, req)
                              let total = Array.length(failing) + Array.length(passing)
                              let rate = if total > 0 { Int.toFloat(Array.length(passing)) /. Int.toFloat(total) *. 100.0 } else { 0.0 }
                              div(
                                list{Attrs.class_("space-y-4")},
                                list{
                                  // Header
                                  div(
                                    ...
                                    list{
                                      div(
                                        ...
                                        list{
                                          div(list{Attrs.class_("text-sm font-medium text-gray-200")}, list{text(label)}),
                                          div(
                                            list{Attrs.class_("text-xs text-gray-500")},
                                            list{text(`${Int.toString(Array.length(passing))} passing, ${Int.toString(Array.length(failing))} failing out of ${Int.toString(total)}`)},
                                          ),
                                        },
                                      ),
                                      div(
                                        list{Attrs.class_(`text-lg font-mono ${
                                          rate > 90.0 ? "text-green-400" : rate > 50.0 ? "text-amber-400" : "text-red-400"
                                        }`)},
                                      ...
                                      div(
                                        list{
                                          Attrs.class_(`h-full rounded-full transition-all ${
                                            rate > 90.0 ? "bg-green-500" : rate > 50.0 ? "bg-amber-500" : "bg-red-500"
                                          }`),
                                          Attrs.prop("style", `width: ${Float.toFixed(rate, ~digits=0)}%`),
                                        },
                                        list{},
                                      ),
                                    },
                                  ...
                                  if Array.length(failing) > 0 {
                                    div(
                                      list{Attrs.class_("space-y-1")},
                                      list{
                                        div(list{Attrs.class_("text-xs text-red-400 font-medium uppercase tracking-wider mb-2")}, list{text("Failing Repos")}),
                                        div(
                                          list{Attrs.class_("max-h-64 overflow-y-auto space-y-1")},
                                          failing->Array.map(a =>
                                            div(
                                              list{Attrs.class_("flex items-center justify-between p-2 bg-red-900/20 border border-red-900/40 rounded text-sm")},
                                              list{
                                                span(list{Attrs.class_("text-gray-300 truncate")}, list{text(a.repoName)}),
                                                span(list{Attrs.class_("text-xs text-red-400")}, list{text("FAIL")}),
                                              },
                                            )
                                          )->List.fromArray,
                                        ),
                                    ...
                                    div(
                                      list{Attrs.class_("p-4 bg-green-900/20 border border-green-800 rounded text-center")},
                                      list{
                                        div(list{Attrs.class_("text-green-400 text-sm")}, list{text("All repos pass this requirement")}),
                                      },
                                    )
                                  },
                                  ...
                                  if Array.length(passing) > 0 {
                                    div(
                                      ...
                                      list{text(`${Int.toString(Array.length(passing))} repos passing (not shown)`)},
                                    )
                                  } else {
                                    noNode
                                  },
                          ...
                          | None =>
                            div(
                              list{Attrs.class_("flex items-center justify-center h-48 text-gray-600 text-sm")},
                ...
                | RsrLanguagePolicy =>
                  div(
                    list{Attrs.class_("space-y-4")},
                    list{
                      div(list{Attrs.class_("text-sm font-medium text-gray-300")}, list{text("Language Policy Enforcement")}),
                      div(list{Attrs.class_("text-xs text-gray-500 mb-4")}, list{text("Banned languages: TypeScript, Node/npm/bun, Go, Python, Java/Kotlin")}),
                      ...
                      div(
                        ...
                        list{
                          div(
                            list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-4")},
                            list{
                              div(list{Attrs.class_("text-xs text-red-400 font-medium mb-2")}, list{text("TypeScript (.ts/.tsx)")}),
                              div(list{Attrs.class_("text-xs text-gray-400")}, list{text("Replacement: ReScript")}),
                              div(list{Attrs.class_("text-xs text-gray-500 mt-1")}, list{text("CI: ts-blocker.yml active")}),
                            },
                          ),
                          div(
                            list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-4")},
                            list{
                              div(list{Attrs.class_("text-xs text-red-400 font-medium mb-2")}, list{text("npm / Bun")}),
                              div(list{Attrs.class_("text-xs text-gray-400")}, list{text("Replacement: Deno (first), Bun (fallback)")}),
                              div(list{Attrs.class_("text-xs text-gray-500 mt-1")}, list{text("CI: npm-bun-blocker.yml active")}),
                            },
                          ),
                          div(
                            list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-4")},
                            list{
                              div(list{Attrs.class_("text-xs text-amber-400 font-medium mb-2")}, list{text("Go (.go)")}),
                              div(list{Attrs.class_("text-xs text-gray-400")}, list{text("Replacement: Rust")}),
                              div(list{Attrs.class_("text-xs text-gray-500 mt-1")}, list{text("Migration: long-term")}),
                            },
                          ),
                          div(
                            list{Attrs.class_("bg-gray-900 border border-gray-700 rounded-lg p-4")},
                            list{
                              div(list{Attrs.class_("text-xs text-amber-400 font-medium mb-2")}, list{text("Python (.py)")}),
                              div(list{Attrs.class_("text-xs text-gray-400")}, list{text("Replacement: Julia / Rust / ReScript")}),
                              div(list{Attrs.class_("text-xs text-gray-500 mt-1")}, list{text("Migration: medium-term")}),
                            },
                          ),
                        },
                      ),
                      div(
                        ...
                        list{text("Allowed: ReScript, Deno, Rust, Gleam, Elixir, Bash, Julia, OCaml, Haskell, Ada, Nickel, Guile Scheme")},
                      ),
                    },
                  )
                },
