...
let renderTransportBadge = (tc: transportClass): Tea_Vdom.t<msg> => {
  ...
  span(
    list{Attrs.class_(`px-2 py-0.5 text-xs font-medium rounded ${colour}`)},
    list{text(label)},
  )
}
  ...
  div(
    ...
    list{
      div(
        ...
        list{
          span(list{Attrs.class_("text-sm font-mono text-gray-300 truncate")}, list{text(result.filePath)}),
...
let renderTabs = (active: protocolSquisherCategory): Tea_Vdom.t<msg> => {
  div(
    ...
    ProtocolSquisherEngine.allCategories->Array.map(tab => {
      ...
      div(
        ...
        list{
          div(
            ...
            list{
              h2(list{Attrs.class_("text-lg font-medium text-gray-200")}, list{text("Protocol-Squisher")}),
              span(list{Attrs.class_("text-xs text-gray-500")}, list{text("13-format schema analysis")}),
              if ps.cliAvailable {
        ...
        list{
              ...
              list{
                ...
                div(
                  ...
                  list{
                    ...
                    button(
                      list{
                        Attrs.class_("px-4 py-2 text-sm bg-cyan-600 text-white rounded hover:bg-cyan-500 disabled:opacity-50"),
                  ...
                  div(
                    ...
                    list{
                      div(
                        ...
                        list{
                          span(
                            list{Attrs.class_(if cmp.compatible { "text-emerald-400 text-sm" } else { "text-red-400 text-sm" })},
                            list{text(if cmp.compatible { "Compatible" } else { "Incompatible" })},
                          ),
                          span(list{Attrs.class_("text-xs text-gray-500")}, list{text(`Adapter cost: ${Int.toString(cmp.adapterCost)}/10`)}),
                        },
                      ),
          ...
          | PsResults =>
            if ps.analysisHistory->Array.length === 0 {
              div(list{Attrs.class_("text-center text-gray-500 mt-8")}, list{text("No analysis results yet. Run an analysis first.")})
            } else {
              div(
          ...
          | PsGuide =>
            div(
              ...
              list{
                h3(list{Attrs.class_("text-base font-medium text-gray-200")}, list{text("Transport Class Guide")}),
                div(
                  list{Attrs.class_("space-y-3")},
                  list{
                    div(list{Attrs.class_("flex items-center gap-3")}, list{renderTransportBadge(Concorde), text("Zero-copy, fixed-size fields, no allocation. Best wire efficiency.")}),
                    div(list{Attrs.class_("flex items-center gap-3")}, list{renderTransportBadge(Business), text("Binary format, schema-driven, minimal overhead.")}),
                    div(list{Attrs.class_("flex items-center gap-3")}, list{renderTransportBadge(Economy), text("Text-based or self-describing. Readable but slower.")}),
                    div(list{Attrs.class_("flex items-center gap-3")}, list{renderTransportBadge(Wheelbarrow), text("Format requires full runtime, significant overhead.")}),
                  },
                ),
                h3(list{Attrs.class_("text-base font-medium text-gray-200 mt-6")}, list{text("Supported Formats")}),
                div(
                  list{Attrs.class_("grid grid-cols-3 gap-2")},
                  ProtocolSquisherEngine.allFormats->Array.map(fmt =>
                    span(list{Attrs.class_("text-gray-300")}, list{text(ProtocolSquisherEngine.formatLabel(fmt))})
                  )->List.fromArray,
                ),
              },
            )
