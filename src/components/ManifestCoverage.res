...
/// Render a tab button.
let tabBtn = (current: manifestCoverageTab, target: manifestCoverageTab, label: string): Tea_Vdom.t<
  msg,
> => {
  ...
  div(
    ...
    list{
          ...
          div(
            ...
            list{
              h2(
                ...
                list{text("Manifest Coverage")},
              ),
              span(
                list{Attrs.class_("text-xs text-gray-500")},
                list{text(`${Int.toString(valid)}/${Int.toString(total)} valid manifests`)},
              ),
            },
          ),
          button(
      ...
      div(
        ...
        ManifestCoverageEngine.allTabs
        ->Array.map(t => tabBtn(state.activeTab, t, ManifestCoverageEngine.tabLabel(t)))
        ->List.fromArray,
      ),
      // Error banner
      switch state.error {
                        ...
                        list{
                          ...
                          if Array.length(repo.validationErrors) > 0 {
                            div(
                              ...
                              list{
                                span(
                                  list{Attrs.class_("text-xs text-gray-500 block mb-1")},
                                  list{text("Errors:")},
                                ),
                                div(
                                  ...
                                  ->Array.map(e =>
                                    div(
                                      list{Attrs.class_("text-xs text-red-300 font-mono pl-2")},
                                      list{text(e)},
                                    )
                                  )
                                  ->List.fromArray,
      ...
      div(
        ...
        list{
          text(
            `${Int.toString(
                ManifestCoverageEngine.missingManifestCount(state.repos),
              )} repos missing manifest`,
          ),
        },
      ),
    },
