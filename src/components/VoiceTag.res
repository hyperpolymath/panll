...
let renderTagBadge = (tagType: mriTagType): Tea_Vdom.t<msg> => {
  ...
  span(
    list{
      Attrs.class_(`text-xs px-1.5 py-0.5 rounded border ${bg} ${text_} ${border} ${isModal ? "ring-1 ring-pink-500/30" : ""}`),
      Attrs.title(VoiceTagEngine.tagTypeLabel(tagType) ++ (isModal ? " (modal — affects system behaviour)" : "")),
    },
    list{text(label)},
  )
}
...
let renderAttribution = (attr: mriAttribution): Tea_Vdom.t<msg> => {
  ...
  span(
    ...
    list{
      span(
        list{Attrs.class_(agentColour)},
        list{text(attr.agent)},
      ),
...
let renderTag = (tag: mriTag, isSelected: bool): Tea_Vdom.t<msg> => {
  let selectedClass = isSelected ? "bg-gray-800/70 border-indigo-600" : "border-gray-800/50 hover:bg-gray-800/30"
  ...
  div(
    list{
      Attrs.class_(`flex items-center gap-2 px-3 py-2 border-b ${selectedClass} ${resolvedClass} transition-colors cursor-pointer`),
    ...
    list{
      ...
      div(
        ...
        list{
          ...
          | Some(m) =>
            span(
              list{Attrs.class_("text-sm text-gray-300 truncate block")},
              list{text(m)},
            )
          | None =>
            span(
              list{Attrs.class_("text-sm text-gray-600 italic")},
              list{text("(no message)")},
            )
          },
        },
      ),
      ...
      if tag.resolved {
        span(
          list{
            Attrs.class_("text-xs text-emerald-600"),
            Attrs.title("Resolved"),
          },
          list{text("R")},
        )
      } else {
      ...
      div(
        ...
        list{
          if !tag.resolved {
            button(
              list{
                Attrs.class_("text-xs px-1.5 py-0.5 text-gray-500 hover:text-emerald-400 hover:bg-gray-800 rounded transition-colors"),
          ...
          button(
            list{
              Attrs.class_("text-xs px-1.5 py-0.5 text-gray-500 hover:text-red-400 hover:bg-gray-800 rounded transition-colors"),
...
let renderVoiceControls = (voice: voiceState): Tea_Vdom.t<msg> => {
  ...
  | VoiceListening => ("Listening...", "text-emerald-400 animate-pulse", "Stop", VoiceTag(StopVoice))
  | VoiceProcessing(transcript) => (`Processing: "${transcript}"`, "text-amber-400", "Cancel", VoiceTag(StopVoice))
  ...
  div(
    ...
    list{
      ...
      button(
        list{
          Attrs.class_("px-3 py-1 text-xs rounded transition-colors " ++ switch voice {
...
let renderSummary = (summary: mriFileSummary): Tea_Vdom.t<msg> => {
  div(
    list{Attrs.class_("flex items-center gap-4 px-4 py-2 border-b border-gray-800 text-xs text-gray-500")},
    list{
...
let renderFilters = (filterType: option<mriTagType>, showResolved: bool): Tea_Vdom.t<msg> => {
  div(
    ...
    list{
      ...
      button(
        list{
          Attrs.class_(`text-xs px-2 py-1 rounded transition-colors ${filterType === None ? "bg-indigo-900/50 text-indigo-300 border border-indigo-700" : "text-gray-500 hover:text-gray-300"}`),
      ...
      button(
        list{
          Attrs.class_(`text-xs px-2 py-1 rounded transition-colors ${filterType === Some(Todo) ? "bg-blue-900/50 text-blue-300 border border-blue-700" : "text-gray-500 hover:text-gray-300"}`),
      ...
      button(
        list{
          Attrs.class_(`text-xs px-2 py-1 rounded transition-colors ${filterType === Some(Fixme) ? "bg-red-900/50 text-red-300 border border-red-700" : "text-gray-500 hover:text-gray-300"}`),
      ...
      button(
        list{
          Attrs.class_(`text-xs px-2 py-1 rounded transition-colors ${filterType === Some(Refactor) ? "bg-amber-900/50 text-amber-300 border border-amber-700" : "text-gray-500 hover:text-gray-300"}`),
      ...
      button(
        list{
          Attrs.class_(`text-xs px-2 py-1 rounded transition-colors ${filterType === Some(Review) ? "bg-cyan-900/50 text-cyan-300 border border-cyan-700" : "text-gray-500 hover:text-gray-300"}`),
      ...
      button(
        list{
          Attrs.class_(`text-xs px-2 py-1 rounded transition-colors ${showResolved ? "bg-gray-700 text-gray-300" : "text-gray-500 hover:text-gray-300"}`),
    ...
    list{
      ...
      div(
        ...
        list{
          ...
          button(
            list{
              Attrs.class_("px-3 py-1.5 text-xs bg-gray-800 text-gray-300 rounded hover:bg-gray-700 border border-gray-700 transition-colors"),
      ...
      div(
        ...
        list{
          div(
            ...
            list{text("No tags yet")},
          ),
          div(
            list{Attrs.class_("text-gray-600 text-sm mb-4")},
            list{text("Use voice commands or the keyboard to annotate code regions. Tags are saved as .mri.json sidecars — portable, no PanLL required.")},
          ),
          div(
