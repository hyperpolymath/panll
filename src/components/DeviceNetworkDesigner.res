      ...
      div(
        ...
        list{
          div(
            ...
            list{
              h2(list{Attrs.class_("text-lg font-bold text-blue-300")}, list{text("Device Network Designer")}),
              span(
                list{Attrs.class_("text-xs text-gray-400")},
                list{text(Int.toString(deviceCount) ++ " devices, " ++ Int.toString(connectionCount) ++ " connections")},
              ),
              span(
                list{
                  Attrs.class_(
                    "text-xs px-2 py-0.5 rounded " ++
                    if state.wiringMode { "bg-blue-700 text-blue-100" } else { "bg-gray-800 text-gray-400" },
                  ),
                },
                list{text(if state.wiringMode { "Wiring ON" } else { "Wiring OFF" })},
              ),
            },
          ),
          button(
        ...
        list{
          button(
            list{
              Attrs.class_(
                ...
                if state.activeTab == Designer { "bg-blue-700 text-white" } else { "bg-gray-800 text-gray-400 hover:text-gray-200" },
              ),
          ...
          button(
            list{
              Attrs.class_(
                "px-3 py-1 text-xs rounded " ++
                if state.activeTab == Devices { "bg-blue-700 text-white" } else { "bg-gray-800 text-gray-400 hover:text-gray-200" },
              ),
          ...
          button(
            list{
              Attrs.class_(
                "px-3 py-1 text-xs rounded " ++
                if state.activeTab == Wiring { "bg-blue-700 text-white" } else { "bg-gray-800 text-gray-400 hover:text-gray-200" },
              ),
          ...
          button(
            list{
              Attrs.class_(
                "px-3 py-1 text-xs rounded " ++
                if state.activeTab == Validation { "bg-blue-700 text-white" } else { "bg-gray-800 text-gray-400 hover:text-gray-200" },
              ),
      ...
      | Some(err) =>
        div(
          ...
          list{
            text(err),
            button(
              list{Attrs.class_("text-red-400 hover:text-red-200 text-xs ml-2"), Events.onClick(DeviceNetworkDesigner(DismissDndError))},
              list{text("Dismiss")},
            ),
          },
        )
            ...
            div(
              ...
              list{
                ...
                div(
                  ...
                  ->Array.map(dt =>
                    span(
                      list{Attrs.class_("px-3 py-1.5 text-xs bg-gray-800 border border-gray-700 text-gray-300 rounded cursor-pointer hover:border-blue-600")},
                      list{text(dt)},
                    )
                  )
                  ->List.fromArray,
                ...
                // Canvas placeholder
                div(
                  list{Attrs.class_("w-full h-48 bg-gray-900 border border-gray-800 rounded flex items-center justify-center")},
                  list{
                    span(
                      list{Attrs.class_("text-gray-600 text-sm")},
                      list{text("Network graph canvas — " ++ Int.toString(deviceCount) ++ " nodes, " ++ Int.toString(connectionCount) ++ " edges")},
                    ),
                  },
                ),
              },
                ...
                div(
                  list{
                    Attrs.class_(
                      ...
                      if isSelected { "bg-blue-900/20 border-blue-700" } else { "bg-gray-900 border-gray-800" },
                    ),
                  },
                  list{
                    div(
                      ...
                      list{
                        div(
                          ...
                          list{
                            span(list{Attrs.class_("text-sm font-bold text-blue-300")}, list{text(d.deviceType)}),
                            span(list{Attrs.class_("text-xs text-gray-500 font-mono")}, list{text(d.id)}),
                          },
                        ),
                        span(
                    ...
                    div(
                      ...
                      list{text("(" ++ Float.toFixed(d.x, ~digits=0) ++ ", " ++ Float.toFixed(d.y, ~digits=0) ++ ")")},
                    ),
                  },
                )
              })
                ...
                div(
                  ...
                  ->Array.map(c => {
                    ...
                    div(
                      list{
                        Attrs.class_(
                          "flex gap-2 text-xs py-1 border-b border-gray-800/30 " ++
                          if isSelected { "bg-blue-900/20" } else { "" },
                        ),
                      },
                      list{
                        span(list{Attrs.class_("w-20 font-mono text-gray-400 truncate")}, list{text(c.fromDevice)}),
                        span(list{Attrs.class_("w-20 font-mono text-gray-400 truncate")}, list{text(c.toDevice)}),
                        ...
                        span(
                          list{Attrs.class_("w-12 " ++ if c.encrypted { "text-green-400" } else { "text-red-400" })},
                          list{text(if c.encrypted { "Yes" } else { "No" })},
                        ),
                      },
                    )
                  })
                ...
                list{
                      ...
                      span(
                        list{
                          Attrs.class_(
                            ...
                            if v.valid { "bg-green-700 text-green-100" } else { "bg-red-700 text-red-100" },
                          ),
                        },
                        list{text(if v.valid { "VALID" } else { "INVALID" })},
                      ),
                      span(list{Attrs.class_("text-xs text-gray-400")}, list{text(Int.toString(v.deviceCount) ++ " devices, " ++ Int.toString(v.connectionCount) ++ " connections")}),
                    },
                  ),
                  // Errors
                    ...
                    div(
                      ...
                      list{
                        ...
                        div(
                          ...
                          v.errors
                          ->Array.map(e =>
                            div(list{Attrs.class_("px-2 py-1 text-xs text-red-200 bg-red-900/30 rounded mb-1")}, list{text(e)})
                          )
                          ->List.fromArray,
                        ),
                      },
                  ...
                  if Array.length(v.warnings) > 0 {
                    div(
                      list{Attrs.class_("space-y-1")},
                      list{
                        h3(list{Attrs.class_("text-sm text-yellow-400 mb-1")}, list{text("Warnings")}),
                        div(
                          list{},
                          v.warnings
                          ->Array.map(w =>
                            div(list{Attrs.class_("px-2 py-1 text-xs text-yellow-200 bg-yellow-900/30 rounded mb-1")}, list{text(w)})
                          )
                          ->List.fromArray,
                        ),
                      },
