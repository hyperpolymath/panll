...
let allCategories: array<verificationDashboardCategory> = [
  VdSummary, VdByLanguage, VdProofs, VdBenchmarks, VdFuzzing,
]
...
let passRateColour = (pct: int): string =>
  if pct >= 95 { "text-emerald-400" }
  else if pct >= 80 { "text-green-400" }
  else if pct >= 60 { "text-amber-400" }
  else { "text-red-400" }

/// Colour for admitted count.
let admittedColour = (count: int): string =>
  if count === 0 { "text-emerald-400" }
  else if count <= 5 { "text-amber-400" }
  else { "text-red-400" }
...
let filterBySearch = (statuses: array<languageVerificationStatus>, query: string): array<languageVerificationStatus> =>
  if query === "" {
    statuses
  } else {
...
let filterDebtOnly = (statuses: array<languageVerificationStatus>): array<languageVerificationStatus> =>
  statuses->Array.filter(s => s.admittedCount > 0)

/// Sort languages by the given criterion.
let sortLanguages = (statuses: array<languageVerificationStatus>, sortBy: verificationSortBy): array<languageVerificationStatus> => {
...
let computeSummary = (statuses: array<languageVerificationStatus>): portfolioVerificationSummary => {
  ...
  let avgPassRate = if totalTests > 0 { totalPassing * 100 / totalTests } else { 0 }
  let languagesWithFuzzing = statuses->Array.filter(s => s.fuzzing !== None)->Array.length
  let languagesFullConformance = statuses->Array.filter(s => s.conformance === FullConformance)->Array.length
  {
...
let allLanguageStatuses: array<languageVerificationStatus> = [
  {
    name: "AffineScript",
    totalTests: 247, passingTests: 241, failingTests: 6, skippedTests: 0,
    admittedCount: 0, provedCount: 12, proofSystems: [Idris2Proof],
    conformance: FullConformance,
    benchmarks: [
      { name: "typecheck-100", language: "AffineScript", meanMs: 12.3, stddevMs: 1.1, iterations: 1000, regression: false },
      { name: "affine-verify", language: "AffineScript", meanMs: 45.7, stddevMs: 3.2, iterations: 500, regression: false },
    ],
    fuzzing: None,
  ...
  {
    name: "Eclexia",
    totalTests: 89, passingTests: 67, failingTests: 22, skippedTests: 0,
    admittedCount: 22, provedCount: 22, proofSystems: [Idris2Proof, CoqProof],
    conformance: FailingConformance,
    benchmarks: [
      { name: "dimensional-check", language: "Eclexia", meanMs: 28.5, stddevMs: 2.8, iterations: 500, regression: true },
    ],
    fuzzing: None,
  ...
  {
    name: "Anvomidav",
    totalTests: 56, passingTests: 48, failingTests: 8, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FullConformance,
    benchmarks: [
      { name: "array-mul-1k", language: "Anvomidav", meanMs: 3.2, stddevMs: 0.4, iterations: 2000, regression: false },
    ],
    fuzzing: None,
  ...
  {
    name: "Ephapax",
    totalTests: 134, passingTests: 130, failingTests: 4, skippedTests: 0,
    admittedCount: 0, provedCount: 8, proofSystems: [Idris2Proof],
    conformance: FullConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "WokeLang",
    totalTests: 0, passingTests: 0, failingTests: 0, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
  ...
  {
    name: "BetLang",
    totalTests: 45, passingTests: 38, failingTests: 7, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FailingConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "Tangle",
    totalTests: 112, passingTests: 108, failingTests: 4, skippedTests: 0,
    admittedCount: 0, provedCount: 15, proofSystems: [Idris2Proof, LeanProof],
    conformance: FullConformance,
    benchmarks: [
      { name: "braid-compose", language: "Tangle", meanMs: 8.1, stddevMs: 0.9, iterations: 1000, regression: false },
      { name: "jones-polynomial", language: "Tangle", meanMs: 156.3, stddevMs: 12.4, iterations: 200, regression: false },
    ],
    fuzzing: Some({ language: "Tangle", targets: 12, linesCovered: 3400, totalLines: 5200, crashesFound: 0, fuzzHours: 48.0 }),
    lastRun: Some("2026-03-13T10:00:00Z"),
  },
  {
    name: "My-Lang",
    totalTests: 78, passingTests: 72, failingTests: 6, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FullConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "Crank",
    totalTests: 34, passingTests: 30, failingTests: 4, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FailingConformance,
    benchmarks: [
      { name: "stack-ops-10k", language: "Crank", meanMs: 1.8, stddevMs: 0.2, iterations: 5000, regression: false },
    ],
    fuzzing: None,
  ...
  {
    name: "Delimit",
    totalTests: 98, passingTests: 91, failingTests: 5, skippedTests: 2,
    admittedCount: 2, provedCount: 11, proofSystems: [Idris2Proof],
    conformance: FullConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "HexSweep",
    totalTests: 23, passingTests: 19, failingTests: 4, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FailingConformance,
    benchmarks: [
      { name: "hex-fill-100x100", language: "HexSweep", meanMs: 34.2, stddevMs: 4.1, iterations: 300, regression: false },
    ],
    fuzzing: None,
  ...
  {
    name: "Cascade",
    totalTests: 41, passingTests: 35, failingTests: 6, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FailingConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "Polytope",
    totalTests: 76, passingTests: 68, failingTests: 8, skippedTests: 0,
    admittedCount: 5, provedCount: 18, proofSystems: [Idris2Proof, CoqProof],
    conformance: FullConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "Coda",
    totalTests: 28, passingTests: 22, failingTests: 6, skippedTests: 0,
    admittedCount: 0, provedCount: 0, proofSystems: [],
    conformance: FailingConformance,
    benchmarks: [],
    fuzzing: None,
  ...
  {
    name: "Strata",
    totalTests: 145, passingTests: 138, failingTests: 5, skippedTests: 2,
    admittedCount: 3, provedCount: 24, proofSystems: [Idris2Proof, LeanProof, CoqProof],
    conformance: FullConformance,
    benchmarks: [
      { name: "universe-check", language: "Strata", meanMs: 67.8, stddevMs: 5.3, iterations: 500, regression: false },
      { name: "stratification", language: "Strata", meanMs: 23.1, stddevMs: 2.0, iterations: 1000, regression: false },
    ],
    fuzzing: Some({ language: "Strata", targets: 8, linesCovered: 4100, totalLines: 6800, crashesFound: 1, fuzzHours: 72.0 }),
