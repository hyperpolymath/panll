...
let renderDriftAura = (orbital: orbitalState, humidity: humidityLevel): Tea_Vdom.t<msg> => {
  ...
  let colourClass = orbital.driftAuraColour === "indigo" ? "bg-indigo-900" : "bg-amber-900"

  div(
    list{
      Attrs.class_(
        `fixed inset-0 ${colourClass} ${opacityClass} transition-all duration-1000 pointer-events-none`,
      ),
    },
    list{},
  )
...
> => {
  if !visible {
    ...
    div(list{Attrs.class_("flex-1 overflow-auto")}, list{PaneL.view(paneL, proofs)})
  }
}
...
let renderQuickPanel = (panel: panelMeta): Tea_Vdom.t<msg> => {
  ...
  button(
    list{
      Attrs.class_(
        "bg-gray-900/60 border border-gray-800 rounded-lg p-3 hover:bg-gray-800/80 hover:border-gray-700 transition-all text-left group",
      ),
    ...
    list{
      div(
        ...
        list{
          ...
          span(
            list{Attrs.class_("text-sm text-gray-300 group-hover:text-white transition-colors")},
...
let renderDarkStart = (model: model): Tea_Vdom.t<msg> => {
  ...
  let connectedPanels =
    ...
    ->Array.filter(p => p.connectionStatus === ServiceConnected)
    ->Array.length
  ...
  div(
    ...
    list{
      ...
      div(
        ...
        list{
          div(
            list{Attrs.class_("flex items-end justify-between mb-8")},
            list{
              div(
                list{},
                list{
                  ...
                  div(
                    ...
                    list{text("eNSAID Neurosymbolic Development Environment")},
                  ),
                },
              ),
              button(
                list{
                  Attrs.class_(
                    "px-6 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-500 transition-colors text-sm font-medium",
                  ),
          ...
          div(
            list{Attrs.class_("flex items-center justify-center gap-8 mb-10")},
            list{
              div(
                list{
                  Attrs.class_(
                    "w-16 h-16 rounded-full bg-indigo-600/30 border border-indigo-500/50 flex items-center justify-center",
                  ),
                },
                list{
                  div(
                    list{Attrs.class_("text-indigo-400 text-[10px] font-medium")},
                    list{text("L")},
                  ),
                },
              ),
              ...
              div(
                list{
                  Attrs.class_(
                    "w-16 h-16 rounded-full bg-emerald-600/30 border border-emerald-500/50 flex items-center justify-center",
                  ),
                },
                list{
                  div(
                    list{Attrs.class_("text-emerald-400 text-[10px] font-medium")},
                    list{text("N")},
                  ),
                },
              ),
              ...
              div(
                list{
                  Attrs.class_(
                    "w-16 h-16 rounded-full bg-amber-600/30 border border-amber-500/50 flex items-center justify-center",
                  ),
                },
                list{
                  div(
                    list{Attrs.class_("text-amber-400 text-[10px] font-medium")},
                    list{text("W")},
                  ),
                },
              ),
          ...
          // System health cards
          div(
            list{Attrs.class_("grid grid-cols-4 gap-3 mb-8")},
            list{
              // Panels
              div(
                list{Attrs.class_("bg-gray-900/60 border border-gray-800 rounded-lg p-4")},
                list{
                  div(
                  ...
                  div(
                    ...
                    list{text(Int.toString(totalPanels))},
                  ),
                  div(
                    list{Attrs.class_("text-xs text-green-500 mt-1")},
                    list{text(`${Int.toString(connectedPanels)} connected`)},
                  ),
                },
              ),
              // Hypatia
              div(
                ...
                list{
                  ...
                  div(
                    list{
                      Attrs.class_(
                        `text-2xl font-light ${if hypatiaConfidence > 0.8 {
                            "text-green-400"
                          } else if hypatiaConfidence > 0.5 {
                            "text-amber-400"
                          } else {
                            "text-gray-500"
                          }}`,
                    ...
                    list{
                      text(
                        if hypatiaConfidence > 0.0 {
                          `${Float.toFixed(hypatiaConfidence *. 100.0, ~digits=0)}%`
                        } else {
                          "---"
                        },
                      ),
              ...
              div(
                ...
                list{
                  ...
                  div(
                    list{Attrs.class_("text-2xl font-light text-gray-200")},
                    list{text(Int.toString(bojCartridges))},
                  ),
                  div(
                    list{Attrs.class_("text-xs text-green-500 mt-1")},
                    list{text(`${Int.toString(bojLoaded)} loaded`)},
                  ),
                },
              ),
              // Sessions
              div(
                ...
                list{
                  ...
                  div(
                    list{Attrs.class_("text-2xl font-light text-gray-200")},
                    list{text(Int.toString(sessionCount))},
                  ),
                  div(
                    list{Attrs.class_("text-xs text-gray-600 mt-1")},
                    list{text("workspace sessions")},
                  ),
                },
              ),
            },
          ...
          div(
            ...
            list{
              div(
                ...
                list{text("Quick Access")},
              ),
              div(
                list{Attrs.class_("grid grid-cols-4 gap-2")},
                model.panelSwitcher.panels
                ...
                div(
                  ...
                  ->Array.map(session =>
                    div(
                      list{
                        Attrs.class_(
                          "flex items-center gap-3 p-2 bg-gray-900/40 rounded hover:bg-gray-800/60 transition-colors",
                        ),
                      },
                      list{
                        div(list{Attrs.class_("w-1.5 h-1.5 rounded-full bg-gray-600")}, list{}),
                        span(
                          list{Attrs.class_("text-sm text-gray-400 flex-1")},
                          list{text(session.name)},
                        ),
                        span(
                          list{Attrs.class_("text-xs text-gray-600")},
                          list{text(Date.make()->Date.toLocaleDateString)},
                        ),
                      },
                    )
                  )
          ...
          // Footer
          div(
            list{Attrs.class_("text-center py-6 border-t border-gray-900")},
            list{
              div(
                list{Attrs.class_("text-xs text-gray-700")},
                list{text("Press Enter or click \"Enter Environment\" to begin")},
              ),
            },
          ),
        },
...
let renderCircuitLines = (
  ...
  if !(paneLVisible && paneNVisible && paneWVisible) {
    ...
    let pulseClass = if orbital.syncHealth > 0.7 {
      ""
    } else {
      "animate-pulse"
    }
    ...
    Tea_Svg.svg(
      list{
        Tea_Svg.Attrs.class_(
          `absolute inset-0 w-full h-full pointer-events-none z-20 ${pulseClass}`,
        ),
...
let renderActivePanel = (model: model): Tea_Vdom.t<msg> => {
  ...
  | Some(PanelContractileCompleteness) =>
    ContractileCompleteness.view(model.contractileCompleteness)
  ...
  | Some(PanelGsaServerBrowser) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("GSA Server Browser — panel loading...")},
    )
  | Some(PanelGsaConfigEditor) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("GSA Config Editor — panel loading...")},
    )
  | Some(PanelGsaServerActions) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("GSA Server Actions — panel loading...")},
    )
  | Some(PanelGsaLiveLogs) =>
    div(list{Attrs.class_("p-4 text-gray-400")}, list{text("GSA Live Logs — panel loading...")})
  | Some(PanelGsaHealthDashboard) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("GSA Health Dashboard — panel loading...")},
    )
  | Some(PanelGsaConfigHistory) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("GSA Config History — panel loading...")},
    )
  | Some(PanelGsaCrossSearch) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("GSA Cross-Search — panel loading...")},
    )
  ...
  | Some(PanelIdaptikServerStatus) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("IDApTIK Server Status — panel loading...")},
    )
  | Some(PanelIdaptikSessionMonitor) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("IDApTIK Session Monitor — panel loading...")},
    )
  | Some(PanelIdaptikPlayerOverview) =>
    div(
      list{Attrs.class_("p-4 text-gray-400")},
      list{text("IDApTIK Player Overview — panel loading...")},
    )
  }
}
...
let view = (model: model): Tea_Vdom.t<msg> => {
  ...
  if model.viewMode === DarkStart {
    ...
    div(
      list{
        Attrs.class_(
          `h-screen ${rootColourClasses(
              model.viewMode,
            )} flex flex-col ${AccessibilityEngine.rootClasses(model.accessibility)}`,
        ),
      },
      list{
        // ────────────────────────────────────────────────────────────────
        ...
        nav(
          list{
            Attrs.class_(
              "sr-only focus-within:not-sr-only focus-within:fixed focus-within:top-0 focus-within:left-0 focus-within:z-50 focus-within:bg-gray-900 focus-within:p-2 focus-within:flex focus-within:gap-2",
            ),
          ...
          list{
            a(
              list{
                Attrs.href("#pane-l"),
                Attrs.class_(
                  "text-blue-400 underline focus:outline-2 focus:outline-blue-400 px-2 py-1 rounded",
                ),
              },
        ...
        div(
          list{
            Attrs.class_(
              `flex-1 flex overflow-hidden relative z-10 ${if model.panelBarVisible {
                  "pr-12"
                } else {
                  ""
                }}`,
          ...
          list{
            ...
            div(
              list{
                Attrs.id("pane-l"),
                Attrs.class_(
                  `flex-1 overflow-auto relative transition-opacity duration-500 ${FocusDimmingEngine.panelOpacityClass(
                      model.focusDimming,
                      "paneL",
                    )}`,
                ),
              ...
              list{
                ...
                CaptureBar.view(
                  "paneL",
                  false,
                  model.capture.captureBarVisible && model.paneLVisible,
                ),
              },
            ),
            ...
            div(
              ...
              list{
                ...
                CaptureBar.view(
                  "paneN",
                  false,
                  model.capture.captureBarVisible && model.paneNVisible,
                ),
              },
            ),
            ...
            div(
              list{
                ...
                Attrs.role("region"),
                Attrs.ariaLabel(
                  "Panel-W — World barycentre, validated output, and VeriSimDB tools",
                ),
              ...
              list{
                ...
                CaptureBar.view(
                  "paneW",
                  false,
                  model.capture.captureBarVisible && model.paneWVisible,
                ),
              },
            ),
          },
