...
PanLL’s permanent custom implementation of The Elm Architecture (TEA)
for ReScript. This is NOT rescript-tea — it’s a purpose-built runtime
with 18 modules supporting keyed diffing, fragments, SVG, 80+
attributes, 30+ events, HTTP, JSON, keyboard, mouse, window, SSR, debug,
and testing.
...
* `+list{}+` syntax for vdom children:
`+Tea_Html.div(list{}, list{...})+`
* `+Events.onClick+` / `+Events.onInput+` for events (NOT
`+Events.onCheck+`)
