...
let pending = (tagger: result<string, string> => 'msg): Tea_Cmd.t<'msg> => {
...
let approve = (eventId: string, tagger: result<string, string> => 'msg): Tea_Cmd.t<'msg> => {
  Tea_Cmd.call(callbacks => {
...
let deny = (eventId: string, reason: string, tagger: result<string, string> => 'msg): Tea_Cmd.t<
  'msg,
> => {
  Tea_Cmd.call(callbacks => {
