...
let loadTags = (filePath: string, tagger: result<string, string> => 'msg): Tea_Cmd.t<'msg> => {
  Tea_Cmd.call(callbacks => {
    let sidecarPath = filePath ++ ".mri.json"
    let _ =
      invoke("voicetag_load", {"path": sidecarPath})
...
let saveTags = (
  ...
  Tea_Cmd.call(callbacks => {
    let sidecarPath = filePath ++ ".mri.json"
    let _ =
      invoke("voicetag_save", {"path": sidecarPath, "content": jsonContent})
      ->Promise.thenResolve(result => {
        callbacks.enqueue(tagger(Ok(result)))
      })
      ->Promise.catch(_err => {
...
let deleteSidecar = (filePath: string, tagger: result<string, string> => 'msg): Tea_Cmd.t<'msg> => {
  Tea_Cmd.call(callbacks => {
    let sidecarPath = filePath ++ ".mri.json"
    let _ =
      invoke("voicetag_delete", {"path": sidecarPath})
      ->Promise.thenResolve(result => {
        callbacks.enqueue(tagger(Ok(result)))
      })
      ->Promise.catch(_err => {
...
let scanProject = (dirPath: string, tagger: result<string, string> => 'msg): Tea_Cmd.t<'msg> => {
  Tea_Cmd.call(callbacks => {
    let _ =
      invoke("voicetag_scan", {"path": dirPath})
      ->Promise.thenResolve(result => {
        callbacks.enqueue(tagger(Ok(result)))
      })
      ->Promise.catch(_err => {
