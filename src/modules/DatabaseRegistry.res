...
let verisimdb: moduleConfig = {
  ...
  playground: Some({
    ...
    exampleQueries: [
      ...
      {
        label: "Graph query",
        query: "SELECT GRAPH.* FROM HEXAD 'entity-123'",
        isDependentType: false,
      },
      {
        label: "Full-text search",
        query: "SEARCH TEXT 'example query' LIMIT 20",
        isDependentType: false,
      },
      {label: "Show drift", query: "SHOW DRIFT", isDependentType: false},
      {
        label: "Existence proof",
        query: "SELECT SEMANTIC.* FROM HEXAD 'entity-1' PROOF EXISTENCE(entity-1)",
        isDependentType: true,
      },
      {
        label: "Multi-proof",
        query: "SELECT * FROM octads PROOF EXISTENCE(e) AND INTEGRITY(e) AND FRESHNESS(e) LIMIT 5",
        isDependentType: true,
      },
    ],
...
let quandledb: moduleConfig = {
  ...
  capabilities: [QueryExecution, Playground],
  playground: Some({
    languageName: "KQL",
    ...
    exampleQueries: [
      ...
      {
        label: "Compute invariant",
        query: "COMPUTE INVARIANT FOR KNOT '3_1'",
        isDependentType: false,
      },
    ],
