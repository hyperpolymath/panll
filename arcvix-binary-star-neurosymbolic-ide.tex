...
Architecture for state management, a Rust/Tauri~2.0 backend, and an
...
\begin{enumerate}
  ...
  \item An implementation report covering the ReScript/Tauri/BEAM stack
...
callback-based side effects. Commands (Tauri IPC calls, HTTP
...
the substrate is implemented in Rust within the Tauri~2.0 backend,
...
\begin{itemize}
  ...
  \item \textbf{IPC bridge:} Tauri's command invocation system,
...
Backend & Rust + Tauri 2.0 & Native IPC, file I/O \\
...
\begin{itemize}
  ...
  \item 60+ Tauri command modules bridging frontend to backend.
  ...
  \item A Rust backend with 27+ Tauri command handlers.
...
\begin{enumerate}
  ...
  \item A \textbf{Command} module wrapping Tauri IPC calls
...
files---using the \tea{} pattern in ReScript with a Rust/Tauri
...
that compiles to JavaScript, the Tauri project for a credible
Electron alternative, the Elixir and BEAM communities for
...
Data \& Protocols & 7 & VeriSimDB Feeds, Protocol Squisher,
  Compatibility Matrix, Databases, VQL Engine \\
