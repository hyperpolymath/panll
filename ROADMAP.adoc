// SPDX-License-Identifier: PMPL-1.0-or-later
= PanLL Development Roadmap
:toc:
:toclevels: 3

== Overview

This roadmap outlines the development plan for **PanLL eNSAID** (Environment for NeSy-Agentic Integrated Development), a neurosymbolic agentic development environment built on The Elm Architecture.

== Current Status: v0.1.0 (2026-02-12) - 92% Complete

✅ **Complete TEA Implementation**

* Model-Update-View architecture (custom TEA)
* Command and Subscription management
* Comprehensive test suite (97 JS tests via Deno.test + 12 Rust tests via cargo test, 109 total)
* Modern tooling (Deno.test, Tauri 2.0, ReScript 11.1)
* npm→Deno migration complete
* All UI components functional (PaneL, PaneN, PaneW, Vexometer, FeedbackOTron)
* 8 Tauri backend commands fully implemented (validate_inference, record_vexation_event, get_vexation_index, submit_feedback, import_panic_attacker_report, import_latest_panic_attacker_report, get_panic_attacker_capability, run_panic_attack_ambush)
* Anti-Crash token gating wired into Pane-N ingestion (backend validation hooks)
* Event-chain import from panic-attack (paste + file load) with persistence
* Security tool integration (panic-attack ambush, timeline import)
* Feedback report types wired to backend
* State persistence via localStorage auto-save (Storage.res)
* ARIA accessibility attributes across all components
* Keyboard shortcuts wired (Ctrl+Shift+L/N/B/W + vexation polling)
* Tailwind CSS 4.x build pipeline (Deno)
* Local dev server wired for Tauri devUrl (serve.sh @ :8000)

== Roadmap

=== v0.2.0 - Enhanced Features & Polish (Q1 2026)

==== Completed in v0.1.0
* [x] Complete Tauri desktop app integration
* [x] Functional UI components
  - [x] FeedbackOTron component
  - [x] Vexometer component
  - [x] PaneL/PaneN/PaneW components

==== Must Have
* [ ] Migration to official rescript-tea (deferred from v0.1.0)
* [x] State persistence (localStorage) -- implemented in src/Storage.res
* [ ] Basic settings management
* [ ] Polish UI/UX based on user feedback

==== Should Have
* [x] Keyboard shortcuts implemented (Ctrl+Shift+L/N/B/W) -- src/subscriptions/KeyboardFixed.res
* [ ] Error boundary handling
* [ ] Loading states and transitions
* [x] Accessibility improvements (ARIA labels) -- comprehensive ARIA across all components

==== Could Have
* [ ] Theme support (dark/light modes)
* [ ] Component style customization
* [ ] Animation framework
* [ ] Onboarding tutorial

=== v0.3.0 - Neurosymbolic Core (Q2 2026)

==== Must Have
* [ ] Integration with neurosymbolic reasoning engine
* [ ] Symbolic constraint system
* [ ] Neural network interface layer
* [ ] Hybrid reasoning pipeline

==== Should Have
* [ ] Visualization of reasoning traces
* [ ] Performance monitoring
* [ ] Debugging tools for NeSy operations
* [ ] Example NeSy applications

==== Could Have
* [ ] Multiple reasoning backends
* [ ] Custom constraint languages
* [ ] Interactive reasoning explorer

=== v0.4.0 - Agentic Capabilities (Q3 2026)

==== Must Have
* [ ] Agent architecture framework
* [ ] Multi-agent coordination
* [ ] Task planning and execution
* [ ] Agent communication protocols

==== Should Have
* [ ] Agent lifecycle management
* [ ] Performance metrics per agent
* [ ] Agent debugging tools
* [ ] Pre-built agent templates

==== Could Have
* [ ] Agent marketplace/registry
* [ ] Custom agent builder UI
* [ ] Agent behavior visualization
* [ ] Distributed agent deployment

=== v0.5.0 - Development Tools (Q4 2026)

==== Must Have
* [ ] Code editor integration
* [ ] REPL for interactive development
* [ ] Project scaffolding tools
* [ ] Build and deployment pipeline

==== Should Have
* [ ] Syntax highlighting for DSLs
* [ ] Auto-completion
* [ ] Code navigation
* [ ] Refactoring tools

==== Could Have
* [ ] Visual programming interface
* [ ] Collaborative editing
* [ ] Version control integration
* [ ] CI/CD integration

=== v1.0.0 - Production Ready (Q1 2027)

==== Must Have
* [ ] Comprehensive documentation
* [ ] Production-grade performance
* [ ] Security audit completed
* [ ] Stable API (no breaking changes)
* [ ] Migration guides
* [ ] Example projects and tutorials

==== Should Have
* [ ] Plugin system
* [ ] Extension marketplace
* [ ] Enterprise features
* [ ] Multi-platform support (Windows, macOS, Linux)

==== Could Have
* [ ] Cloud deployment options
* [ ] SaaS offering
* [ ] Mobile companion app
* [ ] Web-based version

== Technical Debt & Maintenance

=== High Priority
* [ ] Increase test coverage to 95%+
* [ ] Add integration tests
* [ ] Performance benchmarks
* [ ] Memory leak detection
* [ ] Security scanning (automated)

=== Medium Priority
* [ ] Code documentation (JSDoc/inline)
* [ ] API documentation generation
* [ ] Contribution guidelines
* [ ] Code of conduct

=== Low Priority
* [ ] Logo and branding
* [ ] Website/landing page
* [ ] Blog/announcement system
* [ ] Community forum

== Research & Exploration

=== Active Research Areas
* Neurosymbolic AI architectures
* Agentic reasoning frameworks
* Human-AI collaboration patterns
* Formal verification for AI systems

=== Experimental Features
* Quantum-inspired optimization
* Multi-modal reasoning
* Explainable AI integration
* Federated learning support

== Success Metrics

=== v0.2.0
* Desktop app runs smoothly on all platforms
* All UI components functional
* 90%+ test coverage

=== v0.3.0
* NeSy reasoning performs faster than baseline
* 10+ example applications demonstrating capabilities
* 95%+ test coverage

=== v0.4.0
* Agents can coordinate on complex tasks
* Performance scales with number of agents
* 95%+ test coverage maintained

=== v1.0.0
* 1000+ GitHub stars
* 100+ production deployments
* Active community (forums, Discord)
* Regular contributor base

== Dependencies & Integration

=== Current Dependencies
* ReScript 11.1.x
* Tauri 2.x
* Deno runtime (tests + build)
* Tailwind CSS 4.x
* @tauri-apps/api 2.x

=== Planned Integrations
* VSCode extension
* JetBrains plugin
* Neovim/Vim plugin
* Emacs mode

== Community & Ecosystem

=== Documentation
* [ ] API reference (auto-generated)
* [ ] Architecture guide
* [ ] Contributing guide
* [ ] Code of conduct
* [ ] Security policy
* [ ] Example gallery

=== Community Building
* [ ] Discord server
* [ ] GitHub Discussions
* [ ] Monthly community calls
* [ ] Conference presentations
* [ ] Academic papers

=== Ecosystem Projects
* [ ] PanLL plugin system
* [ ] Third-party agent marketplace
* [ ] Example project templates
* [ ] Integration libraries

== License & Governance

**License:** PMPL-1.0-or-later (Palimpsest License)

**Governance Model:** Benevolent Dictator for Life (BDFL) with community input

**Decision Making:**
* Major features: RFC process with community feedback
* Bug fixes: Direct commits with review
* Breaking changes: Require RFC + community consensus

== Contact & Links

* **Repository:** https://github.com/hyperpolymath/panll
* **Issues:** https://github.com/hyperpolymath/panll/issues
* **Discussions:** https://github.com/hyperpolymath/panll/discussions
* **Author:** Jonathan D.A. Jewell <jonathan.jewell@open.ac.uk>

---

_Last Updated: 2026-02-12_

_This roadmap is a living document and will be updated as the project evolves._
